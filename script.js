...
function digitRecentGap(rows,digit){const index=(rows||[]).findIndex(row=>row.digits?.includes(digit));return index<0?(rows||[]).length:index;}
...
function buildLagTargetDayRelationLedger(rows,targetDay){
  const latestLagSource=rows?.[1];if(!latestLagSource?.day||!targetDay)return null;
  const rawGap=(DAYS.indexOf(targetDay)-DAYS.indexOf(latestLagSource.day)+7)%7,dayGap=rawGap||7,chrono=rows.slice().reverse(),transitions=[];
  chrono.forEach((source,index)=>{
    if(source.day!==latestLagSource.day)return;
    const sourceTime=dateValue(source.date),target=chrono.slice(index+1).find(candidate=>candidate.day===targetDay&&Math.round((dateValue(candidate.date)-sourceTime)/86400000)===dayGap);
    if(target)transitions.push({source,target,index});
  });
  const n=transitions.length;if(n<6)return null;const cut=Math.floor(n/2),evidence=DIGITS.map(digit=>({digit,activeFamilySet:new Set(),tieFamilySet:new Set(),activeRules:0,tieRules:0,activeHits:0,activeRecentHits:0,hitBands:0}));
  formulaLibrary().forEach(formula=>{
    const hitIndexes=[],bands=new Set();
    transitions.forEach((tr,index)=>{if(new Set(tr.target.digits).has(formula.fn(tr.source.digits))){hitIndexes.push(index);bands.add(Math.min(2,Math.floor(3*index/n)));}});
    const recentHits=hitIndexes.filter(index=>index>=cut).length,active=hitIndexes.length>=3&&recentHits>=1&&bands.size>=2,tie=!active&&hitIndexes.length>=2&&(recentHits>=1||bands.size>=2),row=evidence[formula.fn(latestLagSource.digits)];
    if(active){row.activeFamilySet.add(formula.family);row.activeRules++;row.activeHits+=hitIndexes.length;row.activeRecentHits+=recentHits;row.hitBands+=bands.size;}
    else if(tie){row.tieFamilySet.add(formula.family);row.tieRules++;}
  });
  evidence.forEach(row=>{row.activeFamilies=row.activeFamilySet.size;row.tieFamilies=row.tieFamilySet.size;delete row.activeFamilySet;delete row.tieFamilySet;});
  evidence.sort((a,b)=>b.activeFamilies-a.activeFamilies||b.activeRules-a.activeRules||b.activeRecentHits-a.activeRecentHits||b.activeHits-a.activeHits||b.tieFamilies-a.tieFamilies||a.digit-b.digit);
  return {latestLagSource,sourceDay:latestLagSource.day,targetDay,dayGap,transitions,evidence};
}
...
function buildTargetTwinCrossRouteRecovery(rows,targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision?.structural||routeDecision?.replayValidated||routeDecision?.useIndependent||historyConditioned||crossRouteConcentration||targetDayBridge||nearTieTargetCoverage||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||!replay||replay.position.tests<6||routeDecision.overlap>1)return null;
  const ledger=buildTargetDayRecurrenceLedger(rows,targetDay);if(!ledger||ledger.transitions.length<6)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}]));
  const target=rankedMap(ledger.evidence),position=rankedMap(positionLadder.evidence),independent=rankedMap(independentLattice.anchors);
  const targetCounts=countMap(ledger.latestTarget.digits),twinDigits=Object.keys(targetCounts).map(Number).filter(digit=>targetCounts[digit]>=2&&!formulaLadder.six.includes(digit));
  if(twinDigits.length!==1)return null;
  const twin={digit:twinDigits[0],target:target.get(twinDigits[0]),position:position.get(twinDigits[0]),independent:independent.get(twinDigits[0])};
  if(!twin.target||twin.target.rank>4||twin.target.activeFamilies<4||twin.target.activeRules<5||twin.target.activeRecentHits<10||twin.target.tieFamilies<4||twin.target.maxPositionCount<2)return null;
  const positionSix=new Set(positionLadder.six),independentSix=new Set(independentLattice.six),sharedSix=digit=>positionSix.has(digit)&&independentSix.has(digit);
  const spine=formulaLadder.four.filter(sharedSix).sort((a,b)=>(position.get(a)?.rank||99)-(position.get(b)?.rank||99)||(independent.get(a)?.rank||99)-(independent.get(b)?.rank||99)||a-b).slice(0,2);
  if(spine.length!==2)return null;
  const carries=unique(rows[0]?.digits||[]).filter(digit=>digit!==twin.digit&&!spine.includes(digit)&&sharedSix(digit)).map(digit=>({digit,position:position.get(digit),independent:independent.get(digit),target:target.get(digit)})).sort((a,b)=>(a.position?.rank||99)-(b.position?.rank||99)||(a.independent?.rank||99)-(b.independent?.rank||99)||(a.target?.rank||99)-(b.target?.rank||99)||a.digit-b.digit);
  if(carries.length<2||!carries.slice(0,2).some(row=>!formulaLadder.five.includes(row.digit)))return null;
  const four=[...spine,twin.digit,carries[0].digit],five=[...four,carries[1].digit],six=five.slice();
  const reserveOrder=[...positionLadder.six,...independentLattice.six,...ledger.evidence.map(row=>row.digit)];
  reserveOrder.forEach(digit=>{if(six.length<6&&!six.includes(digit))six.push(digit);});
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,twinCandidate:twin,spineDigits:spine,carryCandidates:carries.slice(0,2),priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),latestTargetDigits:ledger.latestTarget.digits.slice(),targetTransitions:ledger.transitions.length,mode:'target-twin-cross-route-recovery',modeLabel:'Target-Twin Cross-Route Recovery'};
}
function buildPositionBoundaryTargetTwinRecovery(rows,targetDay,selectedCore,formulaLadder,positionLadder,independentLattice,routeDecision,profileRuns,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision?.structural||routeDecision?.useIndependent||routeDecision?.overlap!==3||historyConditioned||crossRouteConcentration||targetDayBridge||nearTieTargetCoverage||dualRouteDigitCoverageRecovery||strongWinCounterRouteHedgeRecovery||nearParitySparseOverlapRecovery||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||!replay||replay.position.tests<7||replay.position.four<replay.independent.four||replay.position.four>replay.independent.four+.01||replay.position.score<replay.independent.score||replay.position.score>replay.independent.score+.03)return null;
  const ledger=buildTargetDayRecurrenceLedger(rows,targetDay);if(!ledger||ledger.transitions.length<8)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}]));
  const target=rankedMap(ledger.evidence),position=rankedMap(positionLadder.evidence),independent=rankedMap(independentLattice.anchors),echo=rankedMap(independentLattice.echoes);
  const hedge=rankedMap(buildFormulaHedge(profileRuns,formulaLadder.five,positionLadder.positionLedger).evidence),sourceCounts=countMap(rows[0]?.digits||[]);
  const boundaryDigit=formulaLadder.six[5],fifthDigit=formulaLadder.five[4];
  const describe=digit=>({digit,target:target.get(digit),position:position.get(digit),independent:independent.get(digit),echo:echo.get(digit),hedge:hedge.get(digit),sourceCount:sourceCounts[digit]||0});
  const boundary=describe(boundaryDigit),displacedFifth=describe(fifthDigit);
  if(boundary.sourceCount<2||!independentLattice.four.includes(boundary.digit)||independentLattice.six.includes(displacedFifth.digit)||boundary.target?.rank>2||boundary.target?.activeFamilies<6||boundary.target?.activeRules<6||boundary.target?.activeRecentHits<10||boundary.independent?.activeFamilies<4||boundary.independent?.activeRules<6||boundary.independent?.activeRecentHits<10||boundary.hedge?.tieProfiles<3||boundary.hedge?.tiePositions<4||boundary.hedge?.runnerPlacements<14||(displacedFifth.target?.activeFamilies||0)>1||(displacedFifth.independent?.activeFamilies||0)>2||(displacedFifth.hedge?.tiePositions||0)>2||boundary.target.activeFamilies<(displacedFifth.target?.activeFamilies||0)+5||boundary.independent.activeFamilies<(displacedFifth.independent?.activeFamilies||0)+2)return null;
  const twinCore={...selectedCore,finalDigits:formulaLadder.six,strongFive:formulaLadder.five,strongFour:formulaLadder.four,formulaLadder},independentTwin=buildIndependentTwinPortfolio(twinCore),formulaTwin=buildTwinPortfolio(twinCore,profileRuns),coreSet=new Set(formulaLadder.four);
  const echoLeader=independentLattice.echoes.find(row=>coreSet.has(row.digit)),echoLeaderRank=echo.get(echoLeader?.digit)?.rank||99,twinCandidate=independentTwin.candidates.find(row=>row.digit===echoLeader?.digit),formulaTwinRow=formulaTwin.pool.find(row=>row.digit===echoLeader?.digit);
  if(!echoLeader||formulaTwin.choices.some(row=>row.digit===echoLeader.digit)||echoLeaderRank>1||echoLeader.tieFamilies<4||echoLeader.activeFamilies<4||target.get(echoLeader.digit)?.rank>3||target.get(echoLeader.digit)?.activeFamilies<6||!twinCandidate||twinCandidate.activeFamilies<3||twinCandidate.activeRules<4||twinCandidate.activeHits<14||twinCandidate.targetFamilies<3||twinCandidate.targetHits<4||!formulaTwinRow||formulaTwinRow.bestShape.tieFamilies<3)return null;
  const four=formulaLadder.four.slice(),five=[...four,boundary.digit],six=[...five,displacedFifth.digit];
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,boundary,displacedFifth,twinCandidateDigit:echoLeader.digit,twinCandidate,priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),latestTargetDigits:ledger.latestTarget.digits.slice(),targetTransitions:ledger.transitions.length,mode:'position-boundary-target-twin-recovery',modeLabel:'Position-Boundary Target + Twin Recovery'};
}
...
function buildLongHorizonEchoRecovery(rows,targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision?.structural||routeDecision?.replayValidated||routeDecision?.useIndependent||historyConditioned||crossRouteConcentration||targetDayBridge||nearTieTargetCoverage||dualRouteDigitCoverageRecovery||strongWinCounterRouteHedgeRecovery||nearParitySparseOverlapRecovery||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||!replay||replay.position.tests<6||routeDecision.overlap!==1||replay.position.four<replay.independent.four+.10||replay.position.score<replay.independent.score+.10)return null;
  const ledger=buildTargetDayRecurrenceLedger(rows,targetDay);if(!ledger||ledger.transitions.length<6)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}]));
  const target=rankedMap(ledger.evidence),anchor=rankedMap(independentLattice.anchors),echo=rankedMap(independentLattice.echoes),position=rankedMap(positionLadder.evidence),primary=new Set(formulaLadder.six),latestSource=new Set(rows[0]?.digits||[]),latestTarget=new Set(ledger.latestTarget.digits);
  const candidates=DIGITS.filter(digit=>!primary.has(digit)&&!latestSource.has(digit)&&!latestTarget.has(digit)).map(digit=>({digit,target:target.get(digit),anchor:anchor.get(digit),echo:echo.get(digit),position:position.get(digit)})).filter(row=>
    row.target?.rank<=4&&row.target.activeFamilies>=2&&row.target.activeRules>=2&&row.target.activeRecentHits>=4&&row.target.maxPositionCount>=2&&row.target.positionCounts.filter(count=>count>0).length>=2&&
    row.anchor?.rank<=6&&row.anchor.activeFamilies>=3&&row.anchor.activeRecentHits>=6&&
    row.echo?.rank<=3&&row.echo.activeFamilies>=3&&row.echo.tieFamilies>=3&&row.echo.tieRecentHits>=3&&row.echo.confluenceFamilies>=5&&
    row.position?.activeProfiles>=1&&row.position.activeFamilies>=1
  );
  if(candidates.length!==1)return null;const candidate=candidates[0],protectedAnchors=new Set(independentLattice.anchors.slice(0,3).map(row=>row.digit)),protectedPositions=new Set(positionLadder.evidence.slice(0,2).map(row=>row.digit));
  const victims=formulaLadder.four.filter(digit=>!protectedAnchors.has(digit)&&!protectedPositions.has(digit)).map(digit=>({digit,target:target.get(digit),anchor:anchor.get(digit),echo:echo.get(digit),position:position.get(digit)})).filter(row=>
    row.target&&row.echo&&candidate.target.rank<row.target.rank&&candidate.echo.rank<row.echo.rank&&
    candidate.target.activeFamilies>=row.target.activeFamilies+1&&candidate.target.activeRules>=row.target.activeRules+1&&
    candidate.echo.tieFamilies>=row.echo.tieFamilies+3&&candidate.echo.confluenceFamilies>=row.echo.confluenceFamilies+1
  ).sort((a,b)=>(b.echo?.rank||99)-(a.echo?.rank||99)||(b.target?.rank||99)-(a.target?.rank||99)||(b.position?.rank||99)-(a.position?.rank||99)||b.digit-a.digit);
  if(victims.length!==1)return null;const victim=victims[0],four=formulaLadder.four.map(digit=>digit===victim.digit?candidate.digit:digit),five=[...four,...formulaLadder.five.filter(digit=>!four.includes(digit)&&digit!==victim.digit)].slice(0,5),six=[...five,victim.digit];
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,candidate,victim,protectedAnchorDigits:[...protectedAnchors],protectedPositionDigits:[...protectedPositions],priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),latestTargetDigits:ledger.latestTarget.digits.slice(),targetTransitions:ledger.transitions.length,mode:'long-horizon-echo-recovery',modeLabel:'Long-Horizon Echo Recovery'};
}
function buildRepeatedSourceNeighborCarryRecovery(rows,targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,profileRuns,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery,longHorizonEchoRecovery){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision?.structural||routeDecision?.replayValidated||routeDecision?.useIndependent||historyConditioned||crossRouteConcentration||targetDayBridge||nearTieTargetCoverage||dualRouteDigitCoverageRecovery||strongWinCounterRouteHedgeRecovery||nearParitySparseOverlapRecovery||longHorizonEchoRecovery||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||!replay||routeDecision.overlap!==3||replay.position.tests<6||replay.position.four<replay.independent.four||replay.position.four>replay.independent.four+.03||replay.position.score<replay.independent.score||replay.position.score>replay.independent.score+.03||replay.position.six<replay.independent.six+.03)return null;
  const ledger=buildTargetDayRecurrenceLedger(rows,targetDay);if(!ledger||ledger.transitions.length<6)return null;
  const counts=countMap(rows[0]?.digits||[]),repeated=Object.keys(counts).map(Number).filter(digit=>counts[digit]===2);
  if(repeated.length!==1||!formulaLadder.four.includes(repeated[0]))return null;
  const repeatDigit=repeated[0],neighbors=[mod10(repeatDigit-1),mod10(repeatDigit+1)],outsideNeighbors=neighbors.filter(digit=>!formulaLadder.six.includes(digit));
  if(outsideNeighbors.length!==1||!neighbors.some(digit=>formulaLadder.four.includes(digit)))return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}])),target=rankedMap(ledger.evidence),anchor=rankedMap(independentLattice.anchors),position=rankedMap(positionLadder.evidence),hedge=rankedMap(buildFormulaHedge(profileRuns,formulaLadder.five,positionLadder.positionLedger).evidence);
  const candidateDigit=outsideNeighbors[0],boundaryDigit=formulaLadder.six[5],victimDigit=formulaLadder.five[4],describe=digit=>({digit,target:target.get(digit),anchor:anchor.get(digit),position:position.get(digit),hedge:hedge.get(digit),gap:digitRecentGap(rows,digit)}),candidate=describe(candidateDigit),boundary=describe(boundaryDigit),victim=describe(victimDigit);
  const boundarySourcePositions=rows[0].digits.map((digit,index)=>digit===boundaryDigit?index:-1).filter(index=>index>=0),candidateTargetBreadth=candidate.target?.positionCounts.filter(value=>value>0).length||0,candidateTargetOccurrences=sum(candidate.target?.positionCounts||[]);
  if(formulaLadder.five.includes(boundaryDigit)||counts[boundaryDigit]!==1||boundarySourcePositions.length!==1||
    boundary.target?.positionCounts[boundarySourcePositions[0]]<3||boundary.target.maxPositionCount!==boundary.target.positionCounts[boundarySourcePositions[0]]||
    boundary.target.tieFamilies<3||boundary.anchor?.rank>5||boundary.anchor.activeFamilies<2||boundary.anchor.activeRules<2||boundary.anchor.activeRecentHits<5||
    boundary.position?.rank>6||boundary.position.tieFamilies<1||counts[victimDigit]||
    boundary.anchor.rank>=victim.anchor.rank||boundary.target.maxPositionCount<victim.target.maxPositionCount+2||boundary.anchor.activeRecentHits<victim.anchor.activeRecentHits+2)return null;
  if(candidate.anchor?.rank>6||candidate.anchor.activeFamilies<2||candidate.anchor.activeRules<2||candidate.anchor.activeHits<8||candidate.anchor.activeRecentHits<4||candidate.anchor.confluenceFamilies<5||
    candidate.position?.rank>7||candidateTargetBreadth<2||candidateTargetOccurrences<2||
    candidate.anchor.rank>=victim.anchor.rank||candidate.anchor.activeHits<victim.anchor.activeHits+2||candidate.anchor.activeRecentHits<victim.anchor.activeRecentHits+1||
    candidate.hedge?.runnerPlacements<victim.hedge?.runnerPlacements||candidate.gap<victim.gap+1)return null;
  const four=formulaLadder.four.slice(),five=[...four,boundaryDigit],six=[...five,candidateDigit];
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,repeatDigit,neighbors,candidate,boundary,victim,boundarySourcePosition:boundarySourcePositions[0],boundarySourcePositionLabel:POS[boundarySourcePositions[0]],priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),latestTargetDigits:ledger.latestTarget.digits.slice(),targetTransitions:ledger.transitions.length,mode:'repeated-source-neighbor-carry-recovery',modeLabel:'Repeat-Neighbor Carry Recovery'};
}
function buildSourceTwinMirrorRecovery(rows,targetDay,selectedCore,formulaLadder,positionLadder,independentLattice,routeDecision,profileRuns,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery,longHorizonEchoRecovery,repeatedSourceNeighborCarryRecovery){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision?.structural||routeDecision?.replayValidated||routeDecision?.useIndependent||historyConditioned||crossRouteConcentration||targetDayBridge||nearTieTargetCoverage||dualRouteDigitCoverageRecovery||strongWinCounterRouteHedgeRecovery||nearParitySparseOverlapRecovery||longHorizonEchoRecovery||repeatedSourceNeighborCarryRecovery||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||routeDecision.overlap!==2||!replay||replay.position.tests<8||replay.independent.four<replay.position.four+.04||replay.independent.four>=replay.position.four+.08||replay.independent.score<replay.position.score+.025||replay.independent.score>=replay.position.score+.035||replay.independent.six<replay.position.six)return null;
  const sourceCounts=countMap(rows[0]?.digits||[]),repeated=Object.keys(sourceCounts).map(Number).filter(digit=>sourceCounts[digit]===2);
  if(repeated.length!==1||!formulaLadder.four.includes(repeated[0]))return null;
  const sourceTwinDigit=repeated[0],candidateDigit=mod10(9-sourceTwinDigit);
  if(formulaLadder.six.includes(candidateDigit)||candidateDigit===sourceTwinDigit)return null;
  const ledger=buildTargetDayRecurrenceLedger(rows,targetDay);if(!ledger||ledger.transitions.length<9)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}])),target=rankedMap(ledger.evidence),position=rankedMap(positionLadder.evidence),anchor=rankedMap(independentLattice.anchors),hedge=rankedMap(buildFormulaHedge(profileRuns,formulaLadder.five,positionLadder.positionLedger).evidence);
  const candidate={digit:candidateDigit,target:target.get(candidateDigit),position:position.get(candidateDigit),anchor:anchor.get(candidateDigit),hedge:hedge.get(candidateDigit)},victim={digit:sourceTwinDigit,target:target.get(sourceTwinDigit),position:position.get(sourceTwinDigit),anchor:anchor.get(sourceTwinDigit),hedge:hedge.get(sourceTwinDigit)};
  if(candidate.target?.rank>3||candidate.target.activeFamilies<7||candidate.target.activeRules<7||candidate.target.activeRecentHits<16||candidate.target.hitBands<18||candidate.target.positionCounts.filter(value=>value>0).length<2||
    candidate.position?.activeProfiles<1||candidate.position.tieFamilies<5||candidate.anchor?.activeFamilies<3||candidate.anchor.activeRules<4||candidate.anchor.activeHits<18||candidate.anchor.activeRecentHits<9||
    candidate.hedge?.tieProfiles<3||candidate.hedge.tiePositions<4||candidate.hedge.tieFamilies<5||candidate.hedge.tieRules<15||
    victim.target?.rank<8||candidate.target.activeFamilies<victim.target.activeFamilies+3||candidate.target.activeRecentHits<victim.target.activeRecentHits+8)return null;
  const four=formulaLadder.four.map(digit=>digit===sourceTwinDigit?candidateDigit:digit),five=[...four,...formulaLadder.five.filter(digit=>!four.includes(digit)&&digit!==sourceTwinDigit)].slice(0,5),six=[...five,sourceTwinDigit];
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  const hypothetical={...selectedCore,rows,finalDigits:six,strongFive:five,strongFour:four,formulaLadder:{...formulaLadder,four,five,six}},independentTwinRow=buildIndependentTwinPortfolio(hypothetical).pool.find(row=>row.digit===candidateDigit),formulaTwinRow=buildTwinPortfolio(hypothetical,profileRuns).pool.find(row=>row.digit===candidateDigit);
  if(!independentTwinRow||independentTwinRow.activeFamilies<3||independentTwinRow.activeRules<3||independentTwinRow.activeHits<9||independentTwinRow.targetFamilies<2||independentTwinRow.targetRules<2||independentTwinRow.targetHits<2||independentTwinRow.bestShape.targetFamilies<2||
    !formulaTwinRow||formulaTwinRow.bestShape.activeFamilies<1||formulaTwinRow.bestShape.activeRules<1||formulaTwinRow.bestShape.tieFamilies<2||formulaTwinRow.bestShape.topPairPlacements<1)return null;
  return {four,five,six,sourceTwinDigit,candidateDigit,candidate,victim,independentTwinRow,formulaTwinRow,priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),latestTargetDigits:ledger.latestTarget.digits.slice(),targetTransitions:ledger.transitions.length,mode:'source-twin-mirror-recovery',modeLabel:'Source-Twin Mirror Recovery'};
}
function buildNearParityTwoStepUnionRecovery(rows,targetDay,formulaLadder,positionLadder,independentLattice,routeDecision){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision.structural||routeDecision.replayValidated||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||routeDecision.overlap!==3||!replay||replay.position.tests<6||Math.abs(replay.position.score-replay.independent.score)>.01||Math.abs(replay.position.four-replay.independent.four)>.03||replay.independent.five<replay.position.five+.01)return null;
  const targetLedger=buildTargetDayRecurrenceLedger(rows,targetDay),lagLedger=buildLagTargetDayRelationLedger(rows,targetDay);
  if(!targetLedger||targetLedger.transitions.length<6||!lagLedger||lagLedger.transitions.length<6)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}])),target=rankedMap(targetLedger.evidence),lag=rankedMap(lagLedger.evidence),independent=rankedMap(independentLattice.anchors),position=rankedMap(positionLadder.evidence),primary=new Set(formulaLadder.six),independentSix=new Set(independentLattice.six),shared=formulaLadder.six.filter(digit=>independentSix.has(digit));
  if(shared.length!==3)return null;
  const latestSet=new Set(rows[0]?.digits||[]),priorSet=new Set(rows[1]?.digits||[]);
  const anchors=independentLattice.five.filter(digit=>!primary.has(digit)&&latestSet.has(digit)&&priorSet.has(digit)).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),independent:independent.get(digit),position:position.get(digit)})).filter(row=>
    row.target?.rank<=3&&row.target.activeFamilies>=4&&row.target.activeRules>=4&&row.target.activeRecentHits>=6&&
    row.independent?.rank<=4&&row.independent.activeFamilies>=4&&row.independent.activeRules>=4&&row.independent.activeRecentHits>=6&&
    row.position&&row.position.activeProfiles===0&&row.position.tieFamilies>=3&&
    row.lag?.rank<=4&&row.lag.activeFamilies>=3&&row.lag.activeRules>=4&&row.lag.activeRecentHits>=7
  ).sort((a,b)=>a.target.rank-b.target.rank||a.independent.rank-b.independent.rank||a.digit-b.digit);
  if(anchors.length!==1)return null;const anchor=anchors[0];
  const echoes=independentLattice.four.filter(digit=>!primary.has(digit)&&digit!==anchor.digit&&priorSet.has(digit)&&!latestSet.has(digit)).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),independent:independent.get(digit),position:position.get(digit)})).filter(row=>
    row.target?.rank<=6&&row.target.activeFamilies>=2&&row.target.tieFamilies>=3&&row.target.activeRecentHits>=3&&
    row.independent?.tieFamilies>=3&&row.independent.tieRules>=3&&row.independent.tieRecentHits>=4&&
    row.position?.activeProfiles>=1&&row.position.tieFamilies>=2&&row.lag?.activeFamilies>=1&&row.lag.activeRules>=1&&row.lag.activeRecentHits>=2
  ).sort((a,b)=>a.target.rank-b.target.rank||b.independent.tieRecentHits-a.independent.tieRecentHits||a.digit-b.digit);
  if(echoes.length!==1)return null;const echo=echoes[0];
  const targetAnchors=formulaLadder.six.filter(digit=>!shared.includes(digit)&&digit!==anchor.digit&&digit!==echo.digit).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),position:position.get(digit),independent:independent.get(digit)})).filter(row=>
    row.target?.rank===1&&row.target.activeFamilies>=5&&row.target.activeRules>=5&&row.target.activeRecentHits>=10&&row.position?.activeProfiles>=1&&row.lag?.activeFamilies>=2
  );
  if(targetAnchors.length!==1)return null;const targetAnchor=targetAnchors[0],four=[...shared,anchor.digit],five=[...four,targetAnchor.digit],six=[...five,echo.digit];
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,sharedDigits:shared,anchor,targetAnchor,echo,priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),lagSourceDigits:rows[1].digits.slice(),lagSourceDay:lagLedger.sourceDay,lagDayGap:lagLedger.dayGap,targetTransitions:targetLedger.transitions.length,lagTransitions:lagLedger.transitions.length,mode:'near-parity-two-step-union',modeLabel:'Near-Parity Two-Step Union'};
}
function buildCurrentActiveSetEvidence(transitions,latest,digit){
  const families=new Set(),rules=[];
  formulaLibrary().forEach(formula=>{
    if(formula.fn(latest.digits)!==digit)return;const stats=formulaSetMembershipStats(transitions,formula);
    if(stats.status==='ACTIVE'){families.add(formula.family);rules.push({...formula,...stats});}
  });
  return {activeFamilies:families.size,activeRules:rules.length,rawActive:rules.some(row=>row.family==='raw'),rules};
}
function buildStructuralConsensusCarryRecovery(rows,targetDay,selectedCore,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration,targetDayBridge){
  const replay=routeDecision?.replay;
  if(!routeDecision?.structural||routeDecision.replayValidated||routeDecision.overlap!==1||!historyConditioned||crossRouteConcentration||targetDayBridge||formulaLadder.mode!=='history-conditioned-independent'||!replay||replay.position.tests<7||replay.independent.four<replay.position.four||replay.independent.five<replay.position.five+.05||replay.independent.six<replay.position.six||replay.independent.score<replay.position.score+.025)return null;
  const targetLedger=buildTargetDayRecurrenceLedger(rows,targetDay),lagLedger=buildLagTargetDayRelationLedger(rows,targetDay);if(!targetLedger||targetLedger.transitions.length<8||!lagLedger||lagLedger.transitions.length<8)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}])),target=rankedMap(targetLedger.evidence),lag=rankedMap(lagLedger.evidence),position=rankedMap(positionLadder.evidence),independent=rankedMap(independentLattice.anchors),echo=rankedMap(independentLattice.echoes),history=rankedMap(historyConditioned.evidence),primary=new Set(formulaLadder.six),independentFour=new Set(independentLattice.four);
  const consensus=positionLadder.four.filter(digit=>independentFour.has(digit)&&!primary.has(digit)).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),position:position.get(digit),independent:independent.get(digit),echo:echo.get(digit)})).filter(row=>
    row.target?.rank===1&&row.target.activeFamilies>=6&&row.target.activeRules>=6&&row.target.activeRecentHits>=12&&row.position?.activeProfiles>=3&&row.position.activePositions>=2&&row.position.activeRules>=8&&row.echo?.rank<=2&&row.echo.activeFamilies>=2&&row.echo.tieFamilies>=3&&row.echo.activeRecentHits>=8&&row.lag?.activeFamilies>=3&&row.lag.activeRules>=4&&row.lag.activeRecentHits>=6
  );
  if(consensus.length!==1)return null;const anchor=consensus[0];
  const victims=formulaLadder.four.filter(digit=>!independentFour.has(digit)).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),echo:echo.get(digit),history:history.get(digit),position:position.get(digit)})).filter(row=>row.target&&row.lag&&row.echo&&row.target.rank>=5&&row.lag.rank>=8&&row.echo.rank>=8);
  if(victims.length!==1)return null;const victim=victims[0],counts=countMap(rows[0]?.digits||[]);
  const carryCandidates=unique(rows[0]?.digits||[]).filter(digit=>counts[digit]===1&&!primary.has(digit)).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),position:position.get(digit),history:history.get(digit),formula:buildCurrentActiveSetEvidence(selectedCore.transitions,selectedCore.latest,digit)})).filter(row=>
    row.history?.rank===formulaLadder.five.length+1&&row.history.valueFamilies>=1&&row.history.valueRecentFamilies>=1&&row.formula.activeFamilies>=2&&row.formula.activeRules>=4&&row.formula.rawActive&&row.target?.activeFamilies>=3&&row.target.activeRecentHits>=6&&row.lag?.activeFamilies>=2&&row.lag.tieFamilies>=3&&row.lag.activeRecentHits>=4&&row.position?.tieFamilies>=2
  );
  if(carryCandidates.length!==1)return null;const carry=carryCandidates[0],four=formulaLadder.four.map(digit=>digit===victim.digit?anchor.digit:digit),five=[...four,carry.digit],retained=formulaLadder.five.find(digit=>digit!==victim.digit&&!five.includes(digit)),six=[...five,retained];
  if(!Number.isInteger(retained)||new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,anchor,victim,carry,retainedDigit:retained,priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestSourceDigits:rows[0].digits.slice(),latestTargetDigits:targetLedger.latestTarget.digits.slice(),targetTransitions:targetLedger.transitions.length,lagTransitions:lagLedger.transitions.length,lagSourceDay:lagLedger.sourceDay,mode:'structural-consensus-carry-recovery',modeLabel:'Structural Consensus Carry Recovery'};
}
function buildOrthogonalHorizonSplitRecovery(rows,targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,secondary){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='position'||routeDecision.structural||routeDecision.replayValidated||routeDecision.overlap!==4||formulaLadder.mode!=='position-coverage'||positionLadder.positionBridgeApplied||!replay||replay.position.tests<6||replay.position.four<replay.independent.four+.15||replay.position.score<replay.independent.score+.15||replay.position.six<replay.independent.six+.10)return null;
  if(new Set([...positionLadder.six,...independentLattice.six]).size!==6)return null;
  const targetLedger=buildTargetDayRecurrenceLedger(rows,targetDay),lagLedger=buildLagTargetDayRelationLedger(rows,targetDay);
  if(!targetLedger||targetLedger.transitions.length<6||!lagLedger||lagLedger.transitions.length<6||!secondary?.evidence)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}])),position=rankedMap(positionLadder.evidence),independent=rankedMap(independentLattice.anchors),target=rankedMap(targetLedger.evidence),lag=rankedMap(lagLedger.evidence),primary=new Set(formulaLadder.six);
  const sharedLeaders=positionLadder.four.filter(digit=>position.get(digit)?.rank<=2&&independent.get(digit)?.rank<=2);
  if(sharedLeaders.length!==2)return null;
  const horizonLeader=targetLedger.evidence.map((row,index)=>({...row,rank:index+1,lag:lag.get(row.digit)})).find(row=>row.rank===1&&row.activeFamilies>=4&&row.activeRules>=4&&row.activeRecentHits>=6&&row.lag?.rank===1&&row.lag.activeFamilies>=4);
  if(!horizonLeader||!primary.has(horizonLeader.digit)||sharedLeaders.includes(horizonLeader.digit))return null;
  const lagCandidate=lagLedger.evidence.map((row,index)=>({...row,rank:index+1,target:target.get(row.digit),independent:independent.get(row.digit),position:position.get(row.digit)})).filter(row=>
    !primary.has(row.digit)&&row.rank<=3&&row.activeFamilies>=3&&row.activeRules>=3&&row.activeHits>=10&&row.activeRecentHits>=5&&row.tieFamilies>=3&&row.target?.activeFamilies>=1&&row.target.activeRules>=1
  ).sort((a,b)=>a.rank-b.rank||a.digit-b.digit)[0];
  if(!lagCandidate)return null;
  const breadthCandidate=secondary.evidence.map((row,index)=>({...row,rank:index+1,target:target.get(row.digit),lag:lag.get(row.digit),independent:independent.get(row.digit),position:position.get(row.digit)})).filter(row=>
    !primary.has(row.digit)&&row.digit!==lagCandidate.digit&&row.rank<=4&&row.tieProfiles>=3&&row.tiePositions>=3&&row.tieFamilies>=3&&row.tieRules>=9&&row.runnerPlacements>=12&&row.runnerPoints>=25&&row.target?.activeFamilies>=1
  ).sort((a,b)=>a.rank-b.rank||a.digit-b.digit)[0];
  if(!breadthCandidate)return null;
  const four=[...sharedLeaders,horizonLeader.digit,lagCandidate.digit],five=[...four,breadthCandidate.digit];
  if(new Set(four).size!==4||new Set(five).size!==5)return null;
  const remaining=formulaLadder.six.filter(digit=>!five.includes(digit)).map(digit=>({digit,rankSum:(position.get(digit)?.rank||20)+(independent.get(digit)?.rank||20)+(target.get(digit)?.rank||20)+(lag.get(digit)?.rank||20)})).sort((a,b)=>a.rankSum-b.rankSum||a.digit-b.digit),six=[...five,remaining[0]?.digit];
  if(new Set(six).size!==6)return null;
  return {four,five,six,sharedLeaders,horizonLeader,lagCandidate,breadthCandidate,retainedSeat:remaining[0],priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestTargetDigits:targetLedger.latestTarget.digits.slice(),targetTransitions:targetLedger.transitions.length,lagTransitions:lagLedger.transitions.length,lagSourceDay:lagLedger.sourceDay,lagDayGap:lagLedger.dayGap,mode:'orthogonal-horizon-split-recovery',modeLabel:'Orthogonal Horizon-Split Recovery'};
}
function buildTargetDayMarginalLedger(rows,targetDay){
  const series=rows.filter(row=>row.day===targetDay),recent=series.slice(0,Math.ceil(series.length/2));
  const evidence=DIGITS.map(digit=>{
    const events=series.filter(row=>row.digits.includes(digit)).length,occurrences=sum(series.map(row=>row.digits.filter(value=>value===digit).length)),recentEvents=recent.filter(row=>row.digits.includes(digit)).length,positionCounts=POS.map((_,position)=>series.filter(row=>row.digits[position]===digit).length);
    return {digit,events,occurrences,recentEvents,eventRate:events/Math.max(1,series.length),recentRate:recentEvents/Math.max(1,recent.length),positionCounts,maxPositionCount:Math.max(...positionCounts)};
  }).sort((a,b)=>b.events-a.events||b.occurrences-a.occurrences||b.maxPositionCount-a.maxPositionCount||b.recentEvents-a.recentEvents||a.digit-b.digit);
  return {series,recent,evidence,targetDay};
}
function buildCollapsedUniverseOrthogonalRecovery(rows,targetDay,formulaLadder,positionLadder,independentLattice,routeDecision){
  const replay=routeDecision?.replay;
  if(routeDecision?.cause!=='replay'||routeDecision.structural||!routeDecision.replayValidated||routeDecision.overlap!==2||formulaLadder.mode!=='replay-independent'||!replay||replay.position.tests<6||replay.independent.four>=.45||replay.independent.score>=.50||replay.independent.six>=.65||replay.position.six>=.50)return null;
  if(new Set([...positionLadder.six,...independentLattice.six]).size!==6)return null;
  const targetLedger=buildTargetDayRecurrenceLedger(rows,targetDay),lagLedger=buildLagTargetDayRelationLedger(rows,targetDay),marginalLedger=buildTargetDayMarginalLedger(rows,targetDay);
  if(!targetLedger||targetLedger.transitions.length<6||!lagLedger||lagLedger.transitions.length<6||marginalLedger.series.length<7)return null;
  const rankedMap=items=>new Map(items.map((row,index)=>[row.digit,{...row,rank:index+1}])),target=rankedMap(targetLedger.evidence),lag=rankedMap(lagLedger.evidence),position=rankedMap(positionLadder.evidence),independent=rankedMap(independentLattice.anchors),primary=new Set(formulaLadder.six);
  const targetAnchors=targetLedger.evidence.slice(0,2).map((row,index)=>({...row,rank:index+1})).filter(row=>primary.has(row.digit)&&formulaLadder.four.includes(row.digit)&&row.activeFamilies>=5&&row.activeRules>=5&&row.activeRecentHits>=8);
  if(targetAnchors.length!==2)return null;
  const lagCandidate=lagLedger.evidence.map((row,index)=>({...row,rank:index+1,target:target.get(row.digit)})).find(row=>row.rank===1&&!primary.has(row.digit)&&row.activeFamilies>=6&&row.activeRules>=6&&row.activeRecentHits>=12&&row.target?.activeFamilies>=1);
  if(!lagCandidate)return null;
  const marginalLeaders=marginalLedger.evidence.filter(row=>row.events===marginalLedger.evidence[0].events&&row.occurrences===marginalLedger.evidence[0].occurrences),marginalCandidate=marginalLeaders.filter(row=>!primary.has(row.digit)&&row.digit!==lagCandidate.digit&&row.eventRate>=.5&&row.recentRate>=.5&&row.maxPositionCount>=3).sort((a,b)=>b.maxPositionCount-a.maxPositionCount||a.digit-b.digit)[0];
  if(!marginalCandidate)return null;
  const consensusCandidates=DIGITS.filter(digit=>!primary.has(digit)&&digit!==lagCandidate.digit&&digit!==marginalCandidate.digit).map(digit=>({digit,target:target.get(digit),lag:lag.get(digit),marginal:marginalLedger.evidence.find(row=>row.digit===digit)})).filter(row=>
    row.target?.rank<=4&&row.target.activeFamilies>=3&&row.target.activeRules>=3&&row.lag?.activeFamilies>=3&&row.lag.activeRules>=3&&row.lag.activeRecentHits>=6&&row.marginal?.eventRate>=.4
  ).sort((a,b)=>a.target.rank-b.target.rank||(a.lag?.rank||99)-(b.lag?.rank||99)||a.digit-b.digit);
  if(consensusCandidates.length!==1)return null;const consensusCandidate=consensusCandidates[0],four=[targetAnchors[0].digit,targetAnchors[1].digit,lagCandidate.digit,marginalCandidate.digit],five=[...four,consensusCandidate.digit];
  const retained=formulaLadder.six.filter(digit=>!five.includes(digit)).map(digit=>({digit,rankSum:(position.get(digit)?.rank||20)+(independent.get(digit)?.rank||20)+(target.get(digit)?.rank||20)+(lag.get(digit)?.rank||20)})).sort((a,b)=>a.rankSum-b.rankSum||a.digit-b.digit)[0],six=[...five,retained?.digit];
  if(new Set(four).size!==4||new Set(five).size!==5||new Set(six).size!==6)return null;
  return {four,five,six,targetAnchors,lagCandidate,marginalCandidate,consensusCandidate,retainedSeat:retained,displacedDigits:formulaLadder.six.filter(digit=>!six.includes(digit)),priorFour:formulaLadder.four.slice(),priorFive:formulaLadder.five.slice(),priorSix:formulaLadder.six.slice(),latestTargetDigits:targetLedger.latestTarget.digits.slice(),targetTransitions:targetLedger.transitions.length,lagTransitions:lagLedger.transitions.length,lagSourceDay:lagLedger.sourceDay,lagDayGap:lagLedger.dayGap,marginalSeries:marginalLedger.series.length,mode:'collapsed-universe-orthogonal-recovery',modeLabel:'Collapsed-Universe Orthogonal Ensemble'};
}
...
function buildPureFormulaReserve(secondary,finalSix){
  const primary=new Set(finalSix),digits=(secondary?.evidence||[]).filter(row=>!primary.has(row.digit)).sort(compareHedgeEvidence).map(row=>row.digit);
  const targetDayReserve=(secondary?.targetDayReserve||[]).filter(digit=>digits.includes(digit)),weakTieSeat=secondary?.weakTieSeat&&!primary.has(secondary.weakTieSeat.digit)?secondary.weakTieSeat:null;
  return {...secondary,digits,bridge:[],alternatives:digits,targetDayReserve,weakTieSeat,pureOnly:true};
}
...
function buildLegacyPrediction(inputRows){
  ...
  let formulaLadder=useIndependent?independentLattice:positionLadder,weakSixSeat=null,boundarySeat=null,corePromotion=null,historyConditioned=null,crossRouteConcentration=null,targetDayBridge=null,nearTieTargetCoverage=null,targetTwinCrossRouteRecovery=null,replayKCounterRouteGuard=null,replayIndependentTargetCarryTwinRecovery=null,collapsedUniverseOrthogonalRecovery=null,structuralConsensusCarryRecovery=null,positionBoundaryTargetTwinRecovery=null,dualRouteDigitCoverageRecovery=null,strongWinCounterRouteHedgeRecovery=null,nearParitySparseOverlapRecovery=null,longHorizonEchoRecovery=null,repeatedSourceNeighborCarryRecovery=null,sourceTwinMirrorRecovery=null,nearParityTwoStepUnionRecovery=null,orthogonalHorizonSplitRecovery=null;
  ...
  if(!useIndependent)targetTwinCrossRouteRecovery=buildTargetTwinCrossRouteRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage);
  if(targetTwinCrossRouteRecovery)formulaLadder={...formulaLadder,six:targetTwinCrossRouteRecovery.six,five:targetTwinCrossRouteRecovery.five,four:targetTwinCrossRouteRecovery.four,targetTwinCrossRouteRecovery,mode:targetTwinCrossRouteRecovery.mode,modeLabel:targetTwinCrossRouteRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Kembar pada hasil ${selectedCore.targetDay} terakhir berada di luar 6D, tetapi lolos empat besar replay target-day. Dua digit formula terkuat menjadi spine; satu carry sumber yang disepakati kedua rute melengkapi 4D, carry lintas-rute kedua menjaga 5D, dan satu reserve bersama menjaga 6D.`};
  if(!targetTwinCrossRouteRecovery&&!crossRouteConcentration)targetDayBridge=buildTargetDayRecurrenceBridge(rows,selectedCore.targetDay,formulaLadder,positionLadder,useIndependent);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&!useIndependent)targetDayBridge=buildDuplicateSourceTargetDayBridge(rows,selectedCore.targetDay,formulaLadder,positionLadder,useIndependent);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&!useIndependent)targetDayBridge=buildDominantTargetDayBridge(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,useIndependent);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&!useIndependent)targetDayBridge=buildMiddlePositionTargetDayRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&useIndependent)targetDayBridge=buildStructuralTargetDayCoreBridge(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&useIndependent)targetDayBridge=buildSplitEvidenceRepeatPositionBridge(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&useIndependent)targetDayBridge=buildStructuralTargetDayAnchorRestoration(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration);
  if(!targetTwinCrossRouteRecovery&&!targetDayBridge&&useIndependent)targetDayBridge=buildStructuralReplayLossRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration);
  ...
  if(!targetDayBridge&&useIndependent&&!replayKCounterRouteGuard&&!replayIndependentTargetCarryTwinRecovery)collapsedUniverseOrthogonalRecovery=buildCollapsedUniverseOrthogonalRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision);
  if(collapsedUniverseOrthogonalRecovery)formulaLadder={...formulaLadder,six:collapsedUniverseOrthogonalRecovery.six,five:collapsedUniverseOrthogonalRecovery.five,four:collapsedUniverseOrthogonalRecovery.four,collapsedUniverseOrthogonalRecovery,mode:collapsedUniverseOrthogonalRecovery.mode,modeLabel:collapsedUniverseOrthogonalRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Rute posisi dan independen ternyata mengunci semesta 6D yang sama, sementara recall absolut keduanya masih rendah; pergantian rute hanya mengubah urutan dan tidak memberi cakupan ortogonal. Dua anchor formula target-day dipertahankan. Pemimpin hubungan ${collapsedUniverseOrthogonalRecovery.lagSourceDay}→${selectedCore.targetDay}, pemimpin frekuensi hari-target lintas ${collapsedUniverseOrthogonalRecovery.marginalSeries} pekan, dan satu konsensus kedua horizon membentuk ulang 5D. Kursi keenam dipilih melalui jumlah peringkat posisi, independen, target-day, dan jeda.`};
  if(!targetDayBridge&&useIndependent&&!replayKCounterRouteGuard&&!replayIndependentTargetCarryTwinRecovery&&!collapsedUniverseOrthogonalRecovery)structuralConsensusCarryRecovery=buildStructuralConsensusCarryRecovery(rows,selectedCore.targetDay,selectedCore,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration,targetDayBridge);
  if(structuralConsensusCarryRecovery)formulaLadder={...formulaLadder,six:structuralConsensusCarryRecovery.six,five:structuralConsensusCarryRecovery.five,four:structuralConsensusCarryRecovery.four,structuralConsensusCarryRecovery,mode:structuralConsensusCarryRecovery.mode,modeLabel:structuralConsensusCarryRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Satu-satunya persetujuan 4D posisi dan independen, ${structuralConsensusCarryRecovery.anchor.digit}, juga memimpin rantai ${selectedCore.targetDay} sehingga dipulihkan ke inti. Nilai sumber tunggal ${structuralConsensusCarryRecovery.carry.digit} berada tepat di batas recurrence mandiri dan lolos raw-carry, ACTIVE set-membership, serta horizon ${structuralConsensusCarryRecovery.lagSourceDay}→${selectedCore.targetDay}; digit itu naik ke 5D, sedangkan ${structuralConsensusCarryRecovery.retainedDigit} tetap dijaga di 6D.`};
  ...
  if(!targetDayBridge&&!useIndependent&&!nearTieTargetCoverage&&!dualRouteDigitCoverageRecovery&&!strongWinCounterRouteHedgeRecovery&&!nearParitySparseOverlapRecovery)longHorizonEchoRecovery=buildLongHorizonEchoRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery);
  if(longHorizonEchoRecovery)formulaLadder={...formulaLadder,six:longHorizonEchoRecovery.six,five:longHorizonEchoRecovery.five,four:longHorizonEchoRecovery.four,longHorizonEchoRecovery,mode:longHorizonEchoRecovery.mode,modeLabel:longHorizonEchoRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Satu kandidat non-latest di luar 6D muncul unik pada empat besar rantai ${selectedCore.targetDay} lintas-minggu dan tiga besar echo keluarga independen. Dua anchor posisi serta tiga anchor independen teratas dilindungi; digit inti yang kalah pada kedua horizon turun ke kursi keenam.`};
  if(!targetDayBridge&&!useIndependent&&!nearTieTargetCoverage&&!dualRouteDigitCoverageRecovery&&!strongWinCounterRouteHedgeRecovery&&!nearParitySparseOverlapRecovery&&!longHorizonEchoRecovery)repeatedSourceNeighborCarryRecovery=buildRepeatedSourceNeighborCarryRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,profileRuns,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery,longHorizonEchoRecovery);
  if(repeatedSourceNeighborCarryRecovery)formulaLadder={...formulaLadder,six:repeatedSourceNeighborCarryRecovery.six,five:repeatedSourceNeighborCarryRecovery.five,four:repeatedSourceNeighborCarryRecovery.four,repeatedSourceNeighborCarryRecovery,mode:repeatedSourceNeighborCarryRecovery.mode,modeLabel:repeatedSourceNeighborCarryRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Sumber memiliki satu digit berulang yang sudah berada di inti. Carry batas 6D selaras dengan recurrence posisi ${repeatedSourceNeighborCarryRecovery.boundarySourcePositionLabel} pada hari ${selectedCore.targetDay} dan mengungguli kursi kelima lama, sehingga naik ke 5D. Satu-satunya tetangga repeat di luar 6D lolos ACTIVE independen, breadth runner-up, serta pembanding jeda dan menjaga kursi keenam.`};
  if(!targetDayBridge&&!useIndependent&&!nearTieTargetCoverage&&!dualRouteDigitCoverageRecovery&&!strongWinCounterRouteHedgeRecovery&&!nearParitySparseOverlapRecovery&&!longHorizonEchoRecovery&&!repeatedSourceNeighborCarryRecovery)sourceTwinMirrorRecovery=buildSourceTwinMirrorRecovery(rows,selectedCore.targetDay,selectedCore,formulaLadder,positionLadder,independentLattice,routeDecision,profileRuns,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery,longHorizonEchoRecovery,repeatedSourceNeighborCarryRecovery);
  if(sourceTwinMirrorRecovery)formulaLadder={...formulaLadder,six:sourceTwinMirrorRecovery.six,five:sourceTwinMirrorRecovery.five,four:sourceTwinMirrorRecovery.four,sourceTwinMirrorRecovery,mode:sourceTwinMirrorRecovery.mode,modeLabel:sourceTwinMirrorRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Replay independen unggul tipis tetapi belum melewati ambang router. Digit kembar sumber ${sourceTwinMirrorRecovery.sourceTwinDigit} memiliki mirror-9 ${sourceTwinMirrorRecovery.candidateDigit} di luar 6D; mirror tersebut berada di lapisan atas target-day, lolos breadth ACTIVE/TIE, dan tervalidasi repeat-family kembar. Mirror naik ke inti, sedangkan digit kembar sumber tetap dijaga di kursi keenam.`};
  if(!targetDayBridge&&!useIndependent&&!nearTieTargetCoverage&&!dualRouteDigitCoverageRecovery&&!strongWinCounterRouteHedgeRecovery&&!nearParitySparseOverlapRecovery&&!longHorizonEchoRecovery&&!repeatedSourceNeighborCarryRecovery&&!sourceTwinMirrorRecovery)nearParityTwoStepUnionRecovery=buildNearParityTwoStepUnionRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision);
  if(nearParityTwoStepUnionRecovery)formulaLadder={...formulaLadder,six:nearParityTwoStepUnionRecovery.six,five:nearParityTwoStepUnionRecovery.five,four:nearParityTwoStepUnionRecovery.four,nearParityTwoStepUnionRecovery,mode:nearParityTwoStepUnionRecovery.mode,modeLabel:nearParityTwoStepUnionRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Replay posisi dan keluarga independen hampir seri serta hanya berbagi tiga digit. Anchor ${nearParityTwoStepUnionRecovery.anchor.digit} lolos ACTIVE pada keluarga independen, rantai ${selectedCore.targetDay}, dan horizon ${nearParityTwoStepUnionRecovery.lagDayGap} hari; pemimpin target-day ${nearParityTwoStepUnionRecovery.targetAnchor.digit} menjaga 5D, sedangkan echo TIE terbaru ${nearParityTwoStepUnionRecovery.echo.digit} menjaga 6D.`};
  if(!targetDayBridge&&!useIndependent&&!nearTieTargetCoverage&&!dualRouteDigitCoverageRecovery&&!strongWinCounterRouteHedgeRecovery&&!nearParitySparseOverlapRecovery&&!longHorizonEchoRecovery&&!repeatedSourceNeighborCarryRecovery&&!sourceTwinMirrorRecovery&&!nearParityTwoStepUnionRecovery)orthogonalHorizonSplitRecovery=buildOrthogonalHorizonSplitRecovery(rows,selectedCore.targetDay,formulaLadder,positionLadder,independentLattice,routeDecision,preTargetSecondary);
  if(orthogonalHorizonSplitRecovery)formulaLadder={...formulaLadder,six:orthogonalHorizonSplitRecovery.six,five:orthogonalHorizonSplitRecovery.five,four:orthogonalHorizonSplitRecovery.four,orthogonalHorizonSplitRecovery,mode:orthogonalHorizonSplitRecovery.mode,modeLabel:orthogonalHorizonSplitRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Kedua rute menyepakati enam digit yang sama, tetapi replay posisi menang dominan. Dua pemimpin lintas-rute tetap menjadi spine; pemimpin bersama rantai ${selectedCore.targetDay} dan horizon ${orthogonalHorizonSplitRecovery.lagSourceDay}→${selectedCore.targetDay} melengkapi tiga anchor. Satu ACTIVE hubungan jeda masuk 4D dan satu runner-up ortogonal lintas-posisi masuk 5D; kursi keenam dipilih kembali melalui jumlah peringkat empat ledger.`};
  if(!targetDayBridge&&!useIndependent&&!nearTieTargetCoverage&&!dualRouteDigitCoverageRecovery&&!strongWinCounterRouteHedgeRecovery&&!nearParitySparseOverlapRecovery&&!longHorizonEchoRecovery&&!repeatedSourceNeighborCarryRecovery&&!sourceTwinMirrorRecovery&&!nearParityTwoStepUnionRecovery&&!orthogonalHorizonSplitRecovery)positionBoundaryTargetTwinRecovery=buildPositionBoundaryTargetTwinRecovery(rows,selectedCore.targetDay,selectedCore,formulaLadder,positionLadder,independentLattice,routeDecision,profileRuns,historyConditioned,crossRouteConcentration,targetDayBridge,nearTieTargetCoverage,dualRouteDigitCoverageRecovery,strongWinCounterRouteHedgeRecovery,nearParitySparseOverlapRecovery);
  if(positionBoundaryTargetTwinRecovery)formulaLadder={...formulaLadder,six:positionBoundaryTargetTwinRecovery.six,five:positionBoundaryTargetTwinRecovery.five,four:positionBoundaryTargetTwinRecovery.four,positionBoundaryTargetTwinRecovery,mode:positionBoundaryTargetTwinRecovery.mode,modeLabel:positionBoundaryTargetTwinRecovery.modeLabel,routeNote:`${formulaLadder.routeNote} Kursi keenam yang berulang pada sumber terbaru juga unggul kuat pada replay target-day, echo independen, dan hedge empat posisi sehingga naik ke 5D. Inti 4D tetap; pemimpin echo di dalam inti mendapat satu kursi kembar setelah repeat-family target-day lolos.`};
  ...
  secondary=buildPureFormulaReserve(secondary,formulaLadder.six);
  if(collapsedUniverseOrthogonalRecovery)secondary={...secondary,counterRoute:false,inversionGuard:true,displacedPrimaryDigits:collapsedUniverseOrthogonalRecovery.displacedDigits.slice()};
  const ak=buildPairs(selectedCore.posRank[0],selectedCore.posRank[1],formulaLadder.six),le=buildPairs(selectedCore.posRank[2],selectedCore.posRank[3],formulaLadder.six);
  ...
  twinPortfolio=applyStructuralTargetTwinSpecialist(twinPortfolio,core,routeDecision);
  twinPortfolio=applyCollapsedUniverseTwinAudit(twinPortfolio,core,collapsedUniverseOrthogonalRecovery);
  if(targetDayBridge)twinPortfolio=applyTargetDayTwinBridge(twinPortfolio,targetDayBridge);
  if(targetTwinCrossRouteRecovery)twinPortfolio=applyTargetTwinCrossRouteTwin(twinPortfolio,targetTwinCrossRouteRecovery);
...
function buildPairs(left,right,allowedDigits){
  ...
  leftRows.forEach((a,ia)=>rightRows.forEach((b,ib)=>{if(allowed&&(!allowed.has(a.digit)||!allowed.has(b.digit)))return;out.push({pair:`${a.digit}${b.digit}`,score:a.score+b.score-.08*(ia+ib)});}));
...
function applyTargetTwinCrossRouteTwin(portfolio,recovery){
  const promoted=portfolio.pool.find(row=>row.digit===recovery?.twinCandidate?.digit),runner=portfolio.candidates.find(row=>row.digit!==recovery?.twinCandidate?.digit);
  if(!promoted||!runner)return portfolio;
  const choices=[promoted,runner].map((row,index)=>({...row,pair:`${row.digit}${row.digit}`,choice:index+1,targetTwinContinuity:row.digit===promoted.digit}));
  return {...portfolio,choices,state:'PENJAGAAN TARGET-TWIN + CARRY LINTAS-RUTE'};
}
function applyPositionBoundaryTargetTwin(portfolio,recovery){
  const promoted=portfolio.pool.find(row=>row.digit===recovery?.twinCandidateDigit),preserved=portfolio.choices.find(row=>row.digit!==recovery?.twinCandidateDigit);
  if(!promoted||!preserved)return portfolio;
  const choices=[promoted,preserved].map((row,index)=>({...row,pair:`${row.digit}${row.digit}`,choice:index+1,targetEchoTwin:row.digit===promoted.digit}));
  return {...portfolio,choices,state:'PENJAGAAN TARGET-DAY + ECHO KEMBAR'};
}
function applySourceTwinMirrorTwin(portfolio,recovery){
  const promoted=portfolio.pool.find(row=>row.digit===recovery?.candidateDigit),preserved=portfolio.choices.find(row=>row.digit!==recovery?.candidateDigit)||portfolio.candidates.find(row=>row.digit!==recovery?.candidateDigit);
  if(!promoted||!preserved)return portfolio;
  const choices=[promoted,preserved].map((row,index)=>({...row,pair:`${row.digit}${row.digit}`,choice:index+1,sourceTwinMirror:row.digit===promoted.digit,repeatFamily:row.digit===promoted.digit}));
  return {...portfolio,choices,state:'PENJAGAAN MIRROR TARGET-DAY + REPEAT-FAMILY'};
}
...
function applyStructuralTargetTwinSpecialist(portfolio,core,routeDecision){
  if(!routeDecision?.structural||!portfolio?.choices?.[1])return portfolio;
  const audit=buildIndependentTwinPortfolio(core),firstDigit=portfolio.choices[0].digit,currentSecond=audit.pool.find(row=>row.digit===portfolio.choices[1].digit),specialist=audit.pool.filter(row=>row.digit!==firstDigit).sort((a,b)=>b.targetFamilies-a.targetFamilies||b.bestShape.targetFamilies-a.bestShape.targetFamilies||b.targetRules-a.targetRules||b.targetHits-a.targetHits||b.activeFamilies-a.activeFamilies||a.digit-b.digit)[0];
  const promote=specialist&&currentSecond&&specialist.digit!==currentSecond.digit&&specialist.targetFamilies>=3&&specialist.bestShape.targetFamilies>=2&&specialist.activeFamilies>=3&&specialist.activeRules>=3&&specialist.activeHits>=currentSecond.activeHits&&specialist.targetFamilies>=currentSecond.targetFamilies+1;
  if(!promote)return portfolio;
  const first=portfolio.pool.find(row=>row.digit===firstDigit)||portfolio.choices[0],promoted=portfolio.pool.find(row=>row.digit===specialist.digit)||specialist,choices=[first,promoted].map((row,index)=>({...row,pair:`${row.digit}${row.digit}`,choice:index+1,targetRepeatSpecialist:row.digit===specialist.digit}));
  return {...portfolio,choices,state:'PENJAGAAN REPEAT-FAMILY + SPESIALIS TARGET'};
}
function applyCollapsedUniverseTwinAudit(portfolio,core,recovery){
  if(!recovery||!portfolio?.pool?.length)return portfolio;
  const repeatPortfolio=buildIndependentTwinPortfolio(core),formulaByDigit=new Map(portfolio.pool.map(row=>[row.digit,row])),repeatByDigit=new Map(repeatPortfolio.pool.map(row=>[row.digit,row]));
  const audited=core.strongFour.map(digit=>{
    const formula=formulaByDigit.get(digit),repeat=repeatByDigit.get(digit),formulaQualified=Boolean(formula&&formula.bestShape.pairedProfiles>=1&&formula.bestShape.activeFamilies>=1&&formula.bestShape.activeRules>=2),repeatQualified=Boolean(repeat&&repeat.activeFamilies>=3&&repeat.activeRules>=3&&repeat.activeHits>=9&&repeat.targetFamilies>=2&&repeat.targetRules>=2),supportStreams=Number(formulaQualified)+Number(repeatQualified);
    return {digit,formula,repeat,formulaQualified,repeatQualified,supportStreams};
  }).filter(row=>row.formula&&row.supportStreams>0).sort((a,b)=>b.supportStreams-a.supportStreams||(b.formula?.bestShape.pairedProfiles||0)-(a.formula?.bestShape.pairedProfiles||0)||(b.repeat?.targetFamilies||0)-(a.repeat?.targetFamilies||0)||(b.repeat?.activeHits||0)-(a.repeat?.activeHits||0)||a.digit-b.digit);
  if(audited.length<2)return {...portfolio,state:'PENJAGAAN KEMBAR • BUKTI BELUM CUKUP',auditNote:'Dua kandidat kembar tetap ditampilkan sebagai penjagaan, tetapi belum ada dua digit yang lolos audit formula-posisi dan repeat-family.'};
  const selected=audited.slice(0,2),lowConfidence=selected.some(row=>row.supportStreams<2),choices=selected.map((row,index)=>({...row.formula,pair:`${row.digit}${row.digit}`,choice:index+1,dualLedgerTwin:true,twinSupportStreams:row.supportStreams,repeatEvidence:row.repeat}));
  return {...portfolio,choices,state:lowConfidence?'PENJAGAAN KEMBAR DUA-LEDGER • KEPERCAYAAN RENDAH':'PENJAGAAN KEMBAR DUA-LEDGER',auditNote:lowConfidence?'Satu pilihan hanya ditopang satu jalur bukti. Pilihan kembar adalah pagar repeat, bukan pernyataan bahwa hasil berikutnya pasti kembar.':'Kedua pilihan lolos bukti formula-posisi dan repeat-family secara terpisah.'};
}
...
function twinChoiceCards(items){return `<div class="twin-choice-grid">${items.map(x=>`<div class="twin-choice"><small>Pilihan kembar ${x.choice} • ${x.dualLedgerTwin?'audit formula + repeat-family':x.sourceTwinMirror?'mirror sumber + repeat-family':x.targetRepeatSpecialist?'spesialis repeat target-day':x.targetTwinContinuity?'kembar target-day berulang':x.targetEchoTwin?'echo + repeat target-day':x.sourceTargetContinuity?'kontinuitas sumber–target':x.repeatPosition?'repeat-position':x.sourceContinuity?'kontinuitas sumber':x.targetDayBridge?'anchor target-day':x.repeatDepth?'kedalaman repeat seluruh replay':'digit formula'}</small><b>${x.pair}</b><span>Bentuk posisi kandidat ${x.bestShape.label} • belum mengunci posisi</span></div>`).join('')}</div>`;}
function renderResult(r){
  ...
  $('output').innerHTML=`
    <div class="result-hero formula-ladder-card">
      ...
      ${r.collapsedUniverseOrthogonalRecovery?`<div class="target-day-bridge-seat"><b>Ensemble ortogonal saat semesta 6D runtuh</b><span>anchor target-day ${r.collapsedUniverseOrthogonalRecovery.targetAnchors.map(row=>row.digit).join(' ')} • pemimpin jeda ${r.collapsedUniverseOrthogonalRecovery.lagCandidate.digit} masuk 4D • frekuensi stabil ${r.collapsedUniverseOrthogonalRecovery.marginalCandidate.digit} masuk 4D • konsensus dua horizon ${r.collapsedUniverseOrthogonalRecovery.consensusCandidate.digit} masuk 5D • ${r.collapsedUniverseOrthogonalRecovery.retainedSeat.digit} menjaga 6D</span></div>`:''}
      ${r.positionBoundaryTargetTwinRecovery?`<div class="target-day-bridge-seat"><b>Pemulihan batas target-day dan kembar</b><span>digit sumber berulang ${r.positionBoundaryTargetTwinRecovery.boundary.digit} naik dari kursi keenam ke 5D • ${r.positionBoundaryTargetTwinRecovery.displacedFifth.digit} tetap dijaga di 6D • echo inti ${r.positionBoundaryTargetTwinRecovery.twinCandidateDigit} masuk pilihan kembar</span></div>`:''}
      ...
      ${r.longHorizonEchoRecovery?`<div class="target-day-bridge-seat"><b>Pemulihan echo lintas-horizon</b><span>kandidat non-latest ${r.longHorizonEchoRecovery.candidate.digit} naik ke 4D • echo independen peringkat ${r.longHorizonEchoRecovery.candidate.echo.rank} • rantai ${r.targetDay} peringkat ${r.longHorizonEchoRecovery.candidate.target.rank} • ${r.longHorizonEchoRecovery.victim.digit} tetap dijaga di 6D</span></div>`:''}
      ${r.repeatedSourceNeighborCarryRecovery?`<div class="target-day-bridge-seat"><b>Pemulihan repeat-neighbor + carry</b><span>repeat inti ${r.repeatedSourceNeighborCarryRecovery.repeatDigit} → tetangga ${r.repeatedSourceNeighborCarryRecovery.candidate.digit} menjaga 6D • carry ${r.repeatedSourceNeighborCarryRecovery.boundary.digit} selaras posisi ${r.repeatedSourceNeighborCarryRecovery.boundarySourcePositionLabel} dan naik ke 5D • ${r.repeatedSourceNeighborCarryRecovery.victim.digit} turun ke cadangan • ${r.repeatedSourceNeighborCarryRecovery.targetTransitions} replay target-day</span></div>`:''}
      ${r.sourceTwinMirrorRecovery?`<div class="target-day-bridge-seat"><b>Pemulihan mirror kembar sumber</b><span>kembar sumber ${r.sourceTwinMirrorRecovery.sourceTwinDigit}${r.sourceTwinMirrorRecovery.sourceTwinDigit} → mirror-9 ${r.sourceTwinMirrorRecovery.candidateDigit} masuk 4D • target-day peringkat ${r.sourceTwinMirrorRecovery.candidate.target.rank} • ${r.sourceTwinMirrorRecovery.independentTwinRow.targetFamilies} keluarga repeat-target menjaga ${r.sourceTwinMirrorRecovery.candidateDigit}${r.sourceTwinMirrorRecovery.candidateDigit} • digit sumber tetap di 6D</span></div>`:''}
      ${r.nearParityTwoStepUnionRecovery?`<div class="target-day-bridge-seat"><b>Penyatuan near-parity lintas dua horizon</b><span>spine bersama ${r.nearParityTwoStepUnionRecovery.sharedDigits.join(' ')} • anchor ACTIVE ${r.nearParityTwoStepUnionRecovery.anchor.digit} masuk 4D • pemimpin ${r.targetDay} ${r.nearParityTwoStepUnionRecovery.targetAnchor.digit} menjaga 5D • echo TIE ${r.nearParityTwoStepUnionRecovery.echo.digit} menjaga 6D • ${r.nearParityTwoStepUnionRecovery.lagTransitions} replay ${r.nearParityTwoStepUnionRecovery.lagSourceDay}→${r.targetDay}</span></div>`:''}
      ${r.orthogonalHorizonSplitRecovery?`<div class="target-day-bridge-seat"><b>Pemisahan horizon ortogonal</b><span>spine lintas-rute ${r.orthogonalHorizonSplitRecovery.sharedLeaders.join(' ')} • pemimpin dua horizon ${r.orthogonalHorizonSplitRecovery.horizonLeader.digit} • ACTIVE jeda ${r.orthogonalHorizonSplitRecovery.lagCandidate.digit} masuk 4D • runner-up lintas-posisi ${r.orthogonalHorizonSplitRecovery.breadthCandidate.digit} masuk 5D • ${r.orthogonalHorizonSplitRecovery.retainedSeat.digit} menjaga 6D</span></div>`:''}
