...
const APP_VERSION = 'IPHOEL Formula Engine V13.32 • Orthogonal Horizon Split';
...
function twinChoiceCards(items){return `<div class="twin-choice-grid">${items.map(x=>`<div class="twin-choice"><small>Pilihan kembar ${x.choice} • ${x.sourceTwinMirror?'mirror sumber + repeat-family':x.targetRepeatSpecialist?'spesialis repeat target-day':x.targetTwinContinuity?'kembar target-day berulang':x.targetEchoTwin?'echo + repeat target-day':x.sourceTargetContinuity?'kontinuitas sumber–target':x.repeatPosition?'repeat-position':x.sourceContinuity?'kontinuitas sumber':x.targetDayBridge?'anchor target-day':x.repeatDepth?'kedalaman repeat seluruh replay':'digit formula'}</small><b>${x.pair}</b><span>Bentuk posisi kandidat ${x.bestShape.label} • belum mengunci posisi</span></div>`).join('')}</div>`;}
function renderResult(r){
  ...
  $('output').innerHTML=`
    ...
    <div class="backup-card hedge-card"><div class="backup-head"><div><small>Pagar Cadangan Murni</small><b>${r.secondary.digits.length} Digit Cadangan</b></div><span class="backup-risk">Di luar 6D utama</span></div>${digitCards(r.secondary.digits,'backup-digit')}<div class="hedge-map"><span>Urutan bukti TIE / runner-up: ${r.secondary.digits.join(' ')}</span>${r.secondary.targetDayReserve?.length?`<span>Runner target-day: ${r.secondary.targetDayReserve.join(' ')}</span>`:''}${r.secondary.counterRoute?`<span>Menjaga jalur posisi yang ditinggalkan</span>`:''}${r.secondary.weakTieSeat?`<span class="weak-tie-seat">TIE posisi lemah: ${r.secondary.weakTieSeat.digit} (${r.secondary.weakTieSeat.positionLabel})</span>`:''}</div><p>Semua cadangan berada di luar enam digit utama. Bagian ini hanya menjaga sinyal lemah yang masih memiliki bukti TIE atau runner-up; tidak ada digit kuat yang dihitung ulang sebagai cadangan.</p></div>
