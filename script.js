...
const APP_VERSION = 'IPHOEL Formula Engine V12.3 • Dual-Layer Backup Portfolio + Shadow Twin';
...
function buildFormulaPrediction(inputRows){
  ...
  // V12.0: router judge melakukan walk-forward pada market/hari aktif. Ia tidak memberi bobot langsung;
  // hanya router yang menang out-of-sample boleh melakukan maksimal satu rescue bersyarat.
  applyWalkForwardRouterJudge(candidate, rows, latest, targetDay);
  ...
  // V12.3: lapisan backup tidak mengubah keputusan utama. Ia merangking lima digit
  // di luar Strong Five menggunakan bukti residual agar pengguna dapat membedakan
  // kandidat borderline dari digit yang benar-benar lemah.
  const backupFiveDigits = chooseBackupFiveDigitsDecisionEngine(candidate, finalDigits, strongFiveDigits, latest, akle);
  const backupTwinDigit = chooseBackupTwinDigit(candidate, backupFiveDigits, twinDigit, latest, akle);
  if(candidate.decisionEngine){candidate.decisionEngine.backup=backupFiveDigits.slice();candidate.decisionEngine.backupTwin=backupTwinDigit;}
  ...
  audit.sourceTransformEmergence = buildSourceTransformEmergenceAudit(candidate.sourceTransformEmergenceAudit);
  audit.walkForwardRouterJudge = buildWalkForwardRouterJudgeAudit(candidate.walkForwardRouterJudgeAudit);
  ...
  audit.backupPortfolio = buildBackupPortfolioAudit(candidate.backupPortfolio);
  audit.worldReplay = buildWorldFormulaReplayAudit(replayProfile, candidate);
  audit.process = buildProcessCards(rows, formulas);
  return {rows, allRows, latest, targetDay, targetAnchor, transitionProfile, twinCycleProfile, marketProfile, replayProfile, formulas, learned, learnedDay, learnedWeekLatest, learnedWeekTarget, candidate, finalDigits, strongFiveDigits, twinDigit, backupFiveDigits, backupTwinDigit, akle, audit};
}
...
function sourceTransformMacroFamily(key){
  key=String(key||'');
  if(key.startsWith('raw_')) return 'raw';
  if(key.startsWith('m9_')||key.startsWith('m10_')) return 'unary_mirror';
  if(key.startsWith('neighbor_')) return 'neighbor';
  if(key.startsWith('sum_')) return 'pair_sum';
  if(key.startsWith('diff_')||key.startsWith('rdiff_')||key.startsWith('abs_')) return 'pair_diff';
  if(key.startsWith('prod_')||key==='outer_prod'||key==='inner_prod') return 'product';
  if(key.startsWith('m9sum_')||key.startsWith('m10sum_')) return 'pair_mirror';
  if(key.startsWith('triad_')) return 'triad';
  if(['total','root','m9root','m10root'].includes(key)) return 'aggregate';
  if(['corner_sum','middle_sum'].includes(key)) return 'chain';
  return key;
}

function sourceTransformTokens(row){
  const d=(row?.digits||[]).slice(0,4);
  if(d.length<4)return [];
  const names=['A','K','L','E'];
  const out=[];
  const add=(key,label,value)=>out.push({key,family:sourceTransformMacroFamily(key),label,digit:mod10(value)});
  d.forEach((v,i)=>{
    add(`raw_${names[i]}`,`raw ${names[i]}`,v);
    add(`m9_${names[i]}`,`mirror9 ${names[i]}`,9-v);
    add(`m10_${names[i]}`,`mirror10 ${names[i]}`,10-v);
    [-2,-1,1,2].forEach(shift=>add(`neighbor_${shift}_${names[i]}`,`${names[i]} ${shift>0?'+':''}${shift}`,v+shift));
  });
  const pairs=[[0,1],[0,2],[0,3],[1,2],[1,3],[2,3]];
  pairs.forEach(([i,j])=>{
    const a=d[i],b=d[j],nm=`${names[i]}${names[j]}`;
    add(`sum_${nm}`,`${nm} sum`,a+b);
    add(`diff_${nm}`,`${nm} diff`,a-b);
    add(`rdiff_${nm}`,`${nm} reverse diff`,b-a);
    add(`abs_${nm}`,`${nm} absolute diff`,Math.abs(a-b));
    add(`prod_${nm}`,`${nm} product`,a*b);
    add(`m9sum_${nm}`,`mirror9 ${nm} sum`,9-mod10(a+b));
    add(`m10sum_${nm}`,`mirror10 ${nm} sum`,10-mod10(a+b));
  });
  [[0,1,2],[0,1,3],[0,2,3],[1,2,3]].forEach(ix=>{
    const nm=ix.map(i=>names[i]).join('');
    add(`triad_${nm}`,`${nm} triad`,ix.reduce((sum,i)=>sum+d[i],0));
  });
  const total=d.reduce((a,b)=>a+b,0), root=digitalRoot(total);
  add('total','total mod10',total);
  add('root','digital root',root);
  add('m9root','mirror9 root',9-root);
  add('m10root','mirror10 root',10-root);
  add('corner_sum','corner A+E',d[0]+d[3]);
  add('middle_sum','middle K+L',d[1]+d[2]);
  add('outer_prod','outer A×E',d[0]*d[3]);
  add('inner_prod','inner K×L',d[1]*d[2]);
  const seen=new Set();
  return out.filter(x=>{if(seen.has(x.key))return false;seen.add(x.key);return true;});
}

function buildSourceTransformEmergenceProfile(rows,latest,targetDay){
  const chrono=(rows||[]).slice().reverse();
  const transitions=[];
  for(let i=0;i<chrono.length-1;i++){
    const source=chrono[i],target=chrono[i+1];
    if(source?.day===latest?.day&&target?.day===targetDay)transitions.push({source,target});
  }
  ...
  transitions.forEach((tr,index)=>{
    ...
    const band=Math.min(2,Math.floor(3*index/Math.max(1,transitions.length)));
    const weight=index+1;
    sourceTransformTokens(tr.source).forEach(t=>{
      if(!stats[t.key])stats[t.key]={key:t.key,family:t.family,label:t.label,hits:0,trials:0,weightedHits:0,totalWeight:0,bands:new Set(),positionHits:[0,0,0,0]};
      const st=stats[t.key];
      st.trials++;st.totalWeight+=weight;
      if(targetSet.has(t.digit)){st.hits++;st.weightedHits+=weight;st.bands.add(band);}
      (tr.target.digits||[]).forEach((actual,p)=>{if(t.digit===actual)st.positionHits[p]++;});
    });
  });
  const formulaQuality=st=>{
    const rate=st.hits/Math.max(1,st.trials);
    const smooth=(st.hits+1)/(st.trials+2);
    const recent=st.weightedHits/Math.max(1,st.totalWeight);
    const sample=Math.min(1,st.trials/7);
    const breadth=0.84+0.08*Math.min(2,st.bands.size);
    return (0.43*rate+0.34*smooth+0.23*recent)*sample*breadth;
  };
  ...
  current.forEach(t=>{
    ...
    const item={...t,...st,quality:q};
    const old=grouped[t.digit][t.family];
    if(!old||item.quality>old.quality||item.hits>old.hits)grouped[t.digit][t.family]=item;
  });
  DIGITS.forEach(d=>{
    ...
    const harmonic=top2.length===2&&top2[0].quality>0&&top2[1].quality>0?2/(1/top2[0].quality+1/top2[1].quality):(top2[0]?.quality||0)*0.65;
    const bandUnion=new Set();arr.slice(0,4).forEach(x=>x.bands.forEach(b=>bandUnion.add(b)));
    const strong=arr.filter(x=>x.hits>=3).length;
    const medium=arr.filter(x=>x.hits>=2).length;
    let st='AUDIT-ONLY';
    if(arr.length>=2&&strong>=1&&medium>=2&&bandUnion.size>=2)st='ACTIVE';
    else if(arr.length>=2&&medium>=2)st='TIE-BREAKER';
    status[d]=st;familyCount[d]=arr.length;precision[d]=harmonic;
    const statusFactor=st==='ACTIVE'?1:st==='TIE-BREAKER'?0.68:0.22;
    score[d]=Math.round(360000*portfolio*statusFactor+120000*harmonic*statusFactor);
    notes[d]=arr.slice(0,5).map(x=>`${x.label} ${x.hits}/${x.trials}`);
    arr.slice(0,5).forEach(x=>x.positionHits.forEach((hits,p)=>{
      if(hits<2)return;
...
function applySourceTransformEmergencePortfolio(candidate,rows,latest,targetDay){
  candidate.sourceTransformEmergenceScore=Array(10).fill(0);
  candidate.sourceTransformEmergenceAudit=null;
  const profile=buildSourceTransformEmergenceProfile(rows,latest,targetDay);
  candidate.sourceTransformEmergenceProfile=profile;
  if(!profile?.transitions?.length)return;
  DIGITS.forEach(d=>{
    candidate.sourceTransformEmergenceScore[d]=profile.score[d]||0;
...
function buildSourceTransformEmergenceAudit(audit){
  if(!audit)return null;
  return {title:audit.title,top:audit.top,detail:audit.detail};
}

function sourceTransformEmergencePairSeeds(candidate,kind){
  const p=candidate?.sourceTransformEmergenceProfile;
  if(!p?.positionRank)return [];
  const left=kind==='AK'?0:2,right=kind==='AK'?1:3;
  const out=[];
...
// V12.0 Walk-Forward Router Judge.
// Semua router dilatih hanya pada transisi source-day→target-day yang terjadi sebelum kasus validasi.
// Hasilnya tidak menambah skor dasar; ia hanya membuka rescue bila ensemble benar-benar menang out-of-sample.
function wfSameDayTransitions(rows,fromDay,toDay){
  const chrono=(rows||[]).slice().reverse(),out=[];
  for(let i=0;i<chrono.length-1;i++)if(chrono[i]?.day===fromDay&&chrono[i+1]?.day===toDay)out.push({source:chrono[i],target:chrono[i+1]});
  return out;
}
function wfNormalize(arr){const max=Math.max(1e-9,...(arr||[0]));return (arr||[]).map(v=>Math.max(0,Number(v)||0)/max);}
function wfFrequency(training){const a=Array(10).fill(0);(training||[]).forEach((tr,i)=>{const w=.78+.44*(i+1)/Math.max(1,training.length);uniqueDigits(tr.target.digits||[]).forEach(d=>a[d]+=w);});return wfNormalize(a);}
function wfPosition(training){const pos=Array.from({length:4},()=>Array(10).fill(0));(training||[]).forEach((tr,i)=>{const w=.78+.44*(i+1)/Math.max(1,training.length);(tr.target.digits||[]).forEach((d,p)=>pos[p][d]+=w);});const norm=pos.map(wfNormalize),score=Array(10).fill(0);norm.forEach(a=>a.forEach((v,d)=>score[d]+=.25*v));return {score:wfNormalize(score),position:norm};}
function wfCarry(training,source){const role=Array(4).fill(0),same=Array(4).fill(0);let den=0;(training||[]).forEach((tr,i)=>{const w=.80+.40*(i+1)/Math.max(1,training.length),set=new Set(tr.target.digits||[]);den+=w;for(let p=0;p<4;p++){if(set.has(tr.source.digits[p]))role[p]+=w;if(tr.source.digits[p]===tr.target.digits[p])same[p]+=w;}});const score=Array(10).fill(0),position=Array.from({length:4},()=>Array(10).fill(0));for(let p=0;p<4;p++){const q=.72*role[p]/Math.max(1e-9,den)+.28*same[p]/Math.max(1e-9,den),d=source?.digits?.[p];if(Number.isInteger(d)){score[d]+=q;position[p][d]=q;}}return {score:wfNormalize(score),position:position.map(wfNormalize),rawRole:role.map((v,p)=>.72*v/Math.max(1e-9,den)+.28*same[p]/Math.max(1e-9,den))};}
function wfTransform(training,source){
  const stats={};(training||[]).forEach((tr,i)=>{const set=new Set(tr.target.digits||[]),w=.76+.48*(i+1)/Math.max(1,training.length);sourceTransformTokens(tr.source).filter(t=>!t.key.startsWith('raw_')).forEach(t=>{const st=stats[t.key]||(stats[t.key]={family:t.family,hits:0,trials:0,hitW:0,totalW:0});st.trials++;st.totalW+=w;if(set.has(t.digit)){st.hits++;st.hitW+=w;}});});
  const familyByDigit=Array.from({length:10},()=>({}));sourceTransformTokens(source).filter(t=>!t.key.startsWith('raw_')).forEach(t=>{const st=stats[t.key];if(!st)return;const rate=st.hits/Math.max(1,st.trials),smooth=(st.hits+1)/(st.trials+2),recent=st.hitW/Math.max(1e-9,st.totalW),q=(.45*rate+.35*smooth+.20*recent)*Math.min(1,st.trials/6);const old=familyByDigit[t.digit][t.family];if(!old||q>old.q)familyByDigit[t.digit][t.family]={q,hits:st.hits};});
  const score=familyByDigit.map(g=>Object.values(g).sort((a,b)=>b.q-a.q).slice(0,5).reduce((z,x,i)=>z+x.q*[1,.64,.40,.24,.14][i],0));return wfNormalize(score);
}
function wfRecall(score,target){const top=DIGITS.slice().sort((a,b)=>(score[b]||0)-(score[a]||0)||a-b).slice(0,6),actual=uniqueDigits(target?.digits||[]);return actual.filter(d=>top.includes(d)).length/Math.max(1,actual.length);}
function buildWalkForwardRouterJudgeProfile(rows,latest,targetDay){
  const transitions=wfSameDayTransitions(rows,latest?.day,targetDay),names=['frequency','position','transform','carry'],validation=[];
  const make=(training,source)=>{const p=wfPosition(training),c=wfCarry(training,source);return {frequency:wfFrequency(training),position:p.score,positionDetail:p.position,transform:wfTransform(training,source),carry:c.score,carryDetail:c};};
  for(let i=3;i<transitions.length;i++){const training=transitions.slice(0,i),scores=make(training,transitions[i].source),recalls={};names.forEach(n=>recalls[n]=wfRecall(scores[n],transitions[i].target));validation.push({scores,recalls,target:transitions[i].target});}
  const stats={};names.forEach(n=>{const vals=validation.map(v=>v.recalls[n]),avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;stats[n]={avg,weight:avg>=.52?Math.pow(Math.max(0,avg-.46),1.35):0};});
  const combine=scores=>{const out=Array(10).fill(0),den=names.reduce((z,n)=>z+(stats[n].weight||0),0);if(!den)return out;names.forEach(n=>(scores[n]||[]).forEach((v,d)=>out[d]+=(stats[n].weight||0)*v));return wfNormalize(out);};
  const current=make(transitions,latest),ensemble=combine(current);let ensembleAvg=0;if(validation.length)ensembleAvg=validation.reduce((z,v)=>z+wfRecall(combine(v.scores),v.target),0)/validation.length;
  const baseAvg=stats.frequency?.avg||0,bestAvg=Math.max(0,...names.map(n=>stats[n].avg||0));let improved=0,worse=0,flat=0;validation.forEach(v=>{const delta=wfRecall(combine(v.scores),v.target)-v.recalls.frequency;if(delta>.01)improved++;else if(delta<-.01)worse++;else flat++;});
  let status='AUDIT-ONLY';if(validation.length>=2&&ensembleAvg>=baseAvg+.025&&ensembleAvg>=bestAvg-.015&&improved>=worse)status='ACTIVE';else if(validation.length>=1&&ensembleAvg>=baseAvg-.015&&improved+flat>=worse)status='TIE-BREAKER';
  const active=names.filter(n=>(stats[n].weight||0)>0),consensus=DIGITS.map(d=>active.reduce((z,n)=>z+(DIGITS.slice().sort((a,b)=>(current[n][b]||0)-(current[n][a]||0)||a-b).slice(0,6).includes(d)?1:0),0));
...
function carryBudgetProfile(profile,carryCap){
  const dist=profile?.carryOverlapDist||{};
  const alpha=0.65;
  const probs=[];let total=0;
  for(let k=0;k<=4;k++){const v=Number(dist[k]||0)+alpha;probs.push(v);total+=v;}
  probs.forEach((v,i)=>probs[i]=v/Math.max(1,total));
  const expected=probs.reduce((s,p,k)=>s+p*k,0);
  let mode=0;for(let k=1;k<probs.length;k++)if(probs[k]>probs[mode])mode=k;
...
function chooseFormulaDigitsDecisionEngine(candidate, latest, akle){
  ...
  const add=(d,amount,reason)=>{
    ...
    score[d]+=amount;
    if(reason&&amount!==0&&reasons[d].length<12)reasons[d].push(reason);
  };
  const traceWidth=d=>new Set((candidate.digitTrace?.[d]||[]).map(x=>x.family)).size;
  const emergence=candidate.sourceTransformEmergenceProfile;
  const maxEmergence=Math.max(1,...(emergence?.score||[1]));
  ...
  DIGITS.forEach(d=>{
    ...
    const es=(candidate.sourceTransformEmergenceScore||[])[d]||0;
    if(es)add(d,0.08*es,`source-transform ${emergence?.status?.[d]||'audit'}`);
  });
  ...
  fullHistoryFormulaGateRegistry().forEach(e=>{
    ...
    if(w<=0)return;
    DIGITS.forEach(d=>{
      const raw=Number(arr[d]||0);if(!raw)return;
      add(d,Math.sign(raw)*Math.min(185000,Math.abs(raw)*0.24),`gate lolos: ${e.label}`);
    });
  });
  DIGITS.forEach(d=>{
    ...
    const generic=0.08*((candidate.centerBridgeScore||[])[d]||0)+0.06*((candidate.boundaryTailScore||[])[d]||0)+0.06*((candidate.postTwinSpreadScore||[])[d]||0)+0.06*((candidate.complementBridgeScore||[])[d]||0);
  ...
  const addPairSupport=(pairs,weight,label)=>{
    (pairs||[]).slice(0,5).forEach((x,i)=>{
      ...
      add(Number(pair[0]),points,`${label} ${pair}`);add(Number(pair[1]),points,`${label} ${pair}`);
    });
  };
  addPairSupport(akle?.ak,2100,'AK gated');addPairSupport(akle?.le,2250,'LE gated');
  ...
  const profile=candidate.marketProfile||{};
  const latestSet=uniqueDigits(latest?.digits||[]);
  const latestLookup=new Set(latestSet);
  ...
  const carryBudget=carryBudgetProfile(profile,carryCap);
  const selected=[];
  const addSelected=d=>{
    d=Number(d);if(!Number.isInteger(d)||d<0||d>9||selected.includes(d)||selected.length>=6)return false;
    if(latestLookup.has(d)&&selected.filter(x=>latestLookup.has(x)).length>=carryCap)return false;
    selected.push(d);return true;
  };
  ...
  let quorumTarget=3,topCoverage=[],topCoverageSet=new Set(),positionCore=new Set();
  if(historyCtx){
    topCoverage=historyCtx.digitRank.slice(0,6).map(x=>x.digit);topCoverageSet=new Set(topCoverage);
    historyCtx.posRank.forEach(r=>{if(r?.[0])positionCore.add(r[0].digit);});
    ...
    while(broadCount<2&&broadOutsiders.length){
      ...
      const victim=selected.slice().reverse().find(d=>!broad(d)&&!positionCore.has(d));
      if(victim==null)break;selected[selected.indexOf(victim)]=incoming;broadCount++;coverageDecision=`${victim}→${incoming} coverage lintas-depth`;
    }
    quorumTarget=historyCtx.n>=6?4:3;
    let quorumCount=selected.filter(d=>topCoverageSet.has(d)).length;
    const incomingQueue=topCoverage.filter(d=>!selected.includes(d)&&historyCtx.positions[d].size>=2&&historyCtx.depthBands[d].size>=2);
    const notes=[];
    while(quorumCount<quorumTarget&&incomingQueue.length){
      const incoming=incomingQueue.shift();
      if(latestLookup.has(incoming)&&selected.filter(d=>latestLookup.has(d)).length>=carryCap)continue;
      const victim=selected.filter(d=>!topCoverageSet.has(d)&&!positionCore.has(d)).sort((a,b)=>(score[a]||0)-(score[b]||0))[0];
  ...
  const maxDecision=Math.max(1,...score),maxCoverage=Math.max(1,...(historyCtx?.coverage||[1]));
  const carryQuality=d=>0.48*(score[d]/maxDecision)+0.24*((historyCtx?.coverage?.[d]||0)/maxCoverage)+0.20*((carryCounts[d]||0)/Math.max(1,...carryCounts))+0.08*((emergence?.score?.[d]||0)/maxEmergence);
  ...
  if(emergence?.rank?.length){
    ...
    picks.slice(0,emergencePickLimit).forEach(incomingObj=>{
      ...
      const projectedCarry=projected.filter(d=>latestLookup.has(d)).length;
      const projectedQuorum=topCoverageSet.size?projected.filter(d=>topCoverageSet.has(d)).length:quorumTarget;
      if(projectedCarry>carryCap||projectedQuorum<quorumTarget)return;
      const victimValue=emergenceValue(victim);
      const carryRelease=latestLookup.has(victim)&&carryNow>carryBudget.preferred;
      const precisionAdvance=(incomingObj.precision||0)>((emergence?.precision?.[victim]||0)*1.01);
  ...
  if(emergence?.rank?.length&&carryJudgeActive&&emergencePromoted.length<2&&canSwap()){
    ...
    if(latestEmergence&&carryNow<carryCap&&carryBudget.expected>=carryNow+0.35){
      const protectedTop=new Set(topCoverage.slice(0,2));
      const victim=selected.filter(d=>!positionCore.has(d)&&!protectedTop.has(d)&&!latestLookup.has(d))
        .sort((a,b)=>emergenceValue(a)-emergenceValue(b)||(score[a]||0)-(score[b]||0))[0];
      if(victim!=null){
        const projected=selected.filter(d=>d!==victim).concat(latestEmergence.digit);
        if((!topCoverageSet.size||projected.filter(d=>topCoverageSet.has(d)).length>=quorumTarget)&&emergenceValue(latestEmergence.digit)>=emergenceValue(victim)*0.78){
          selected[selected.indexOf(victim)]=latestEmergence.digit;
          emergencePromoted.push(latestEmergence.digit);
          swapNotes.push(`${victim}→${latestEmergence.digit} (latest-transform)`);
        }
      }
    }
  }
  ...
  // V12.0 Cross-Router Position Quorum.
  // Digit hanya boleh diselamatkan bila mendapat suara posisi dari sedikitnya dua jalur independen.
  // Jalur yang dipakai: mining full-history, exact-position emergence, AK/LE ordered formula,
  // dan meta-position yang telah berstatus ACTIVE. Maksimal satu rescue.
  const routerJudge=candidate.walkForwardRouterJudge;
  const crossPositionPromoted=[];
  const positionVotes=Array.from({length:10},()=>({points:0,families:new Set(),positions:new Set(),details:[]}));
  const vote=(d,family,p,amount,label)=>{d=Number(d);if(!Number.isInteger(d)||d<0||d>9||amount<=0)return;const v=positionVotes[d];v.points+=amount;v.families.add(family);v.positions.add(p);v.details.push(label);};
  const voteRank=(family,ranks,factor=1,allowDigit=()=>true)=>{(ranks||[]).forEach((rank,p)=>{const values=(rank||[]).map(x=>({digit:Number(x.digit),points:Number(x.points||0)})),max=Math.max(1,...values.map(x=>x.points));values.slice(0,2).forEach((x,i)=>{if(!allowDigit(x.digit)||x.points<=0)return;vote(x.digit,family,p,factor*(x.points/max)*(i===0?1:.68),`${family}-${['A','K','L','E'][p]}#${i+1}`);});});};
  voteRank('history',historyCtx?.posRank,1);
  voteRank('emergence',emergence?.positionRank,.88,d=>(emergence?.status?.[d]||'AUDIT-ONLY')!=='AUDIT-ONLY');
  const pairPositionRanks=Array.from({length:4},()=>[]);
  (akle?.ak||[]).slice(0,3).forEach((x,i)=>{const q=String(x.pair||'');if(/^\d{2}$/.test(q)){pairPositionRanks[0].push({digit:Number(q[0]),points:3-i});pairPositionRanks[1].push({digit:Number(q[1]),points:3-i});}});
  (akle?.le||[]).slice(0,3).forEach((x,i)=>{const q=String(x.pair||'');if(/^\d{2}$/.test(q)){pairPositionRanks[2].push({digit:Number(q[0]),points:3-i});pairPositionRanks[3].push({digit:Number(q[1]),points:3-i});}});
  pairPositionRanks.forEach((arr,p)=>{const merged={};arr.forEach(x=>merged[x.digit]=(merged[x.digit]||0)+x.points);pairPositionRanks[p]=Object.entries(merged).map(([digit,points])=>({digit:Number(digit),points})).sort((a,b)=>b.points-a.points||a.digit-b.digit);});
  voteRank('akle',pairPositionRanks,.72);
  if(routerJudge?.status==='ACTIVE'){
    const metaRanks=(routerJudge.current?.positionDetail||[]).map(a=>DIGITS.map(d=>({digit:d,points:Number(a?.[d]||0)})).sort((x,y)=>y.points-x.points||x.digit-y.digit));
    voteRank('meta-position',metaRanks,.66);
  }
  candidate.crossRouterPositionConsensus={votes:positionVotes.map(v=>({points:v.points,families:[...v.families],positions:[...v.positions],details:v.details.slice()}))};
  const crossValue=d=>{const v=positionVotes[d];return v.points+.28*Math.min(1,v.families.size/3)+.16*Math.min(1,v.positions.size/2)+.14*(score[d]/maxDecision)+.10*((emergence?.score?.[d]||0)/maxEmergence);};
  ...
  if(incomingCross&&canSwap()){
    ...
    for(const victim of victims){
      ...
      if(positionCore.has(victim)&&incomingCross.v.families.size<3)continue;
      if(incomingCross.value<crossValue(victim)+(breadthWin?.06:.18))continue;
      const projected=selected.filter(d=>d!==victim).concat(incoming);
      if(projected.filter(d=>latestLookup.has(d)).length>carryCap)continue;
      if(topCoverageSet.size&&projected.filter(d=>topCoverageSet.has(d)).length<quorumTarget)continue;
      selected[selected.indexOf(victim)]=incoming;crossPositionPromoted.push(incoming);frontierProtected.add(incoming);swapNotes.push(`${victim}→${incoming} (position quorum)`);break;
    }
  }

  // V12.0 Carry-Role Balance.
  ...
  if(routerJudge?.status==='ACTIVE'&&(routerJudge.stats?.carry?.avg||0)>=.55&&canSwap()){
    ...
    if(carryIncoming&&carryNow<carryCap){
      ...
      for(const victim of victims){
        ...
        if(topCoverageSet.size&&projected.filter(d=>topCoverageSet.has(d)).length<quorumTarget)continue;
        const carryUtility=.58*carryIncoming.norm+.24*Math.min(1,carryIncoming.v.points/1.5)+.18*emergenceValue(carryIncoming.digit);
        if(carryUtility<.78*crossValue(victim))continue;
        selected[selected.indexOf(victim)]=carryIncoming.digit;carryRolePromoted.push(carryIncoming.digit);frontierProtected.add(carryIncoming.digit);swapNotes.push(`${victim}→${carryIncoming.digit} (carry role)`);break;
      }
    }
  }
  ...
  if(routerJudge?.status==='ACTIVE'&&canSwap()){
    ...
    if(incoming){
      ...
      for(const victim of victims){
        ...
        if(projected.filter(d=>latestLookup.has(d)).length>carryCap)continue;
        if(topCoverageSet.size&&projected.filter(d=>topCoverageSet.has(d)).length<quorumTarget)continue;
        selected[selected.indexOf(victim)]=incoming.digit;metaPromoted.push(incoming.digit);frontierProtected.add(incoming.digit);swapNotes.push(`${victim}→${incoming.digit} (router judge)`);break;
      }
    }
  }
  ...
  if(swapNotes.length<2&&excluded.length){
    ...
    if(victim!=null&&emergence?.status?.[incoming]==='ACTIVE'&&emergenceValue(incoming)>emergenceValue(victim)*1.18){
      const projected=selected.filter(d=>d!==victim).concat(incoming);
      if(projected.filter(d=>latestLookup.has(d)).length<=carryCap&&(!topCoverageSet.size||projected.filter(d=>topCoverageSet.has(d)).length>=quorumTarget)){
        selected[selected.indexOf(victim)]=incoming;emergencePromoted.push(incoming);swapNotes.push(`${victim}→${incoming} omission`);
      }
    }
  }

  const finalCarry=selected.filter(d=>latestLookup.has(d)).length;
...
// V12.3 Dual-Layer Backup Portfolio.
// Keanggotaan lapisan backup melengkapi Strong Five agar tidak redundan, tetapi urutannya
// BUKAN sekadar urutan terbalik. Urutan dihitung dari bukti residual yang kalah tipis:
// decision score, posisi, emergence, router judge, pasangan AK/LE, carry-role, dan status F6.
function chooseBackupFiveDigitsDecisionEngine(candidate, finalDigits, strongFiveDigits, latest, akle){
  const primary=uniqueDigits(strongFiveDigits||[]).slice(0,5);
  const primarySet=new Set(primary);
  const finalSet=new Set(uniqueDigits(finalDigits||[]));
  const decision=candidate?.decisionEngine?.score||candidate?.routerScore||candidate?.score||Array(10).fill(0);
  const history=candidate?.targetFullHistoryCoverageBalanceBridgeContext;
  const emergence=candidate?.sourceTransformEmergenceProfile;
  const judge=candidate?.walkForwardRouterJudge;
  const latestSet=new Set(uniqueDigits(latest?.digits||[]));
  const maxDecision=Math.max(1,...decision.map(x=>Math.max(0,Number(x)||0)));
  const maxCoverage=Math.max(1,...((history?.coverage)||[1]).map(x=>Math.max(0,Number(x)||0)));
  const maxEmergence=Math.max(1,...((emergence?.score)||[1]).map(x=>Math.max(0,Number(x)||0)));
  const primaryFloor=Math.min(...primary.map(d=>Number(decision[d]||0)).filter(Number.isFinite),maxDecision);
  const top3PositionCount=d=>(history?.posRank||[]).reduce((z,r)=>z+((r||[]).slice(0,3).some(x=>Number(x.digit)===d)?1:0),0);
  const exactPositionCount=d=>(emergence?.positionRank||[]).reduce((z,r)=>z+((r||[]).slice(0,3).some(x=>Number(x.digit)===d&&Number(x.points||0)>0)?1:0),0);
  const pairWeight=d=>{
    let total=0;
    (akle?.ak||[]).slice(0,5).forEach((x,i)=>{const q=String(x.pair||'');if(q.includes(String(d)))total+=(5-i)/5;});
    (akle?.le||[]).slice(0,5).forEach((x,i)=>{const q=String(x.pair||'');if(q.includes(String(d)))total+=(5-i)/5;});
    return Math.min(1,total/2.4);
  };
  const carryRole=d=>{
    if(!latestSet.has(d))return 0;
    const raw=judge?.current?.carry?.[d]||0;
    const hist=(candidate?.marketProfile?.carryDigitCounts||[])[d]||0;
    const maxHist=Math.max(1,...(candidate?.marketProfile?.carryDigitCounts||[1]));
    return .62*Number(raw||0)+.38*Number(hist||0)/maxHist;
  };
  const items=DIGITS.filter(d=>!primarySet.has(d)).map(d=>{
    ...
    const emergenceNorm=Math.max(0,Number(emergence?.score?.[d]||0))/maxEmergence;
    const status=emergence?.status?.[d]||'AUDIT-ONLY';
    const statusFactor=status==='ACTIVE'?1:status==='TIE-BREAKER'?.66:.20;
    const routerNorm=judge?.status==='AUDIT-ONLY'?0:Number(judge?.ensemble?.[d]||0)*(judge?.status==='ACTIVE'?1:.48);
    ...
    const f6Bonus=finalSet.has(d)?.15:0;
    // Kandidat dekat batas Strong Five mendapat bonus uncertainty-corridor. Digit dengan
    // skor hampir nol tetap berada di lapisan B, tetapi ditempatkan paling belakang.
    const cutoffRatio=primaryFloor>0?Math.min(1,Math.max(0,Number(decision[d]||0))/primaryFloor):0;
    const corridor=.12*Math.pow(cutoffRatio,.72);
    const points=.30*decisionNorm+.17*coverageNorm+.16*emergenceNorm*statusFactor+
      .14*positionSupport+.10*pairSupport+.06*routerNorm+.04*carrySupport+f6Bonus+corridor;
    const reasons=[];
    if(finalSet.has(d))reasons.push('digit F6 yang tidak masuk Strong Five');
    if(positionSupport>=.45)reasons.push(`top-posisi ${top3PositionCount(d)} jalur`);
    if(emergenceNorm>=.45)reasons.push(`emergence ${status}`);
    if(pairSupport>=.35)reasons.push('muncul pada AK/LE unggulan');
    if(carrySupport>=.35)reasons.push('carry-role residual');
    if(cutoffRatio>=.72)reasons.push('kalah tipis dari batas utama');
    if(!reasons.length)reasons.push('residual evidence rendah');
    return {digit:d,points,decisionNorm,coverageNorm,emergenceNorm,status,positionSupport,pairSupport,carrySupport,finalSix:finalSet.has(d),reasons};
  }).sort((a,b)=>b.points-a.points||Number(b.finalSix)-Number(a.finalSix)||b.decisionNorm-a.decisionNorm||a.digit-b.digit);
  const backup=items.slice(0,5).map(x=>x.digit);
  candidate.backupPortfolio={
    primary:primary.slice(),backup:backup.slice(),ranked:items,
    note:'Lapisan B melengkapi digit di luar Strong Five. Keanggotaannya anti-overlap; urutannya memakai residual evidence, bukan urutan terbalik sederhana.'
  };
  return backup;
}

// Kembar Backup / Shadow Twin tidak setara dengan kembar utama. Ia mengambil runner-up
// dari seluruh digit (bukan hanya lapisan B) agar repeat yang kalah tipis tetap terlihat.
// Ambangnya lebih rendah, tetapi masih membutuhkan sedikitnya dua jalur bukti dan tetap
// diblokir oleh source-shape cooldown yang kuat.
function chooseBackupTwinDigit(candidate, backupDigits, primaryTwinDigit, latest, akle){
  const audit=candidate?.twinAudit;
  const ranked=(audit?.ranked||[]).filter(x=>Number(x.digit)!==Number(primaryTwinDigit));
  if(!ranked.length)return null;
  const calibration=audit?.repeatShapeCalibration;
  const shapeByDigit={};
  (calibration?.candidates||[]).forEach(x=>{
    const d=Number(x.digit);if(!Number.isInteger(d))return;
    const old=shapeByDigit[d];
    if(!old||Number(x.points||0)>Number(old.points||0))shapeByDigit[d]=x;
  });
  const backupSet=new Set(uniqueDigits(backupDigits||[]));
  const latestSet=new Set(uniqueDigits(latest?.digits||[]));
  const pairRepeat={};
  const addPair=q=>{q=String(q||'');if(!/^\d{2}$/.test(q))return;const c=countMap(q.split('').map(Number));Object.entries(c).forEach(([d,n])=>{if(n>=2)pairRepeat[d]=(pairRepeat[d]||0)+n;});};
  (akle?.ak||[]).slice(0,3).forEach(x=>addPair(x.pair));
  (akle?.le||[]).slice(0,3).forEach(x=>addPair(x.pair));
  const top=Math.max(1,...ranked.map(x=>Number(x.points||0)));
  const candidates=ranked.map(x=>{
    const d=Number(x.digit),shape=shapeByDigit[d];
    const base=Number(x.points||0)/top;
    const shapeEvidence=shape?Math.min(1,.45*Number(shape.posSupport||0)+.30*Math.min(1,Number(shape.shapeRate||0)/.28)+.25*Math.min(1,Number(shape.digitRate||0)/.12)):0;
    const pairEvidence=Math.min(1,Number(pairRepeat[d]||0)/2);
    const carryEvidence=latestSet.has(d)&&Number(calibration?.carryRepeatRate||0)>=.45?.34:0;
    const backupBonus=backupSet.has(d)?.10:0;
    const evidenceCount=(shapeEvidence>=.28?1:0)+(pairEvidence>0?1:0)+(carryEvidence>0?1:0)+((x.reasons||[]).length>=2?1:0);
    const points=.55*base+.24*shapeEvidence+.11*pairEvidence+carryEvidence+backupBonus;
    return {digit:d,points,evidenceCount,shape,base,reasons:x.reasons||[],inBackup:backupSet.has(d)};
  }).sort((a,b)=>b.points-a.points||b.evidenceCount-a.evidenceCount||a.digit-b.digit);
  const chosen=candidates.find(x=>!calibration?.sourceRepeatBlocked&&(
    (x.evidenceCount>=2&&x.points>=.42)||
    (x.base>=.78&&x.points>=.55&&(x.reasons||[]).length>=4)
  ))||null;
  if(candidate.backupPortfolio){
    candidate.backupPortfolio.backupTwin=chosen?.digit??null;
    candidate.backupPortfolio.backupTwinRank=candidates.slice(0,5);
    candidate.backupPortfolio.backupTwinNote=chosen?'Kembar backup adalah runner-up berisiko tinggi; bukan pengganti kembar utama.':'Tidak ada runner-up repeat yang mencapai bukti minimum.';
  }
  return chosen?.digit??null;
}

function buildBackupPortfolioAudit(portfolio){
  if(!portfolio)return null;
  return {
    title:'V12.3 Dual-Layer Backup Portfolio',
    ...
    backupTwin:portfolio.backupTwin==null?'Tidak ada kembar backup layak':`${portfolio.backupTwin}${portfolio.backupTwin}`,
...
function buildDecisionEngineAudit(audit){
  ...
  return {
    title:'Decision Engine V12.3: protected core → repeat/carry arbitration → dual-layer backup',
...
function buildRepeatShapeCalibration(candidate,allowedDigits,akle,latest){
  ...
  (akle?.ak||[]).slice(0,4).forEach((x,i)=>{const q=String(x.pair||'');if(/^\d{2}$/.test(q)){pairArrays[0][Number(q[0])]+=4-i;pairArrays[1][Number(q[1])]+=4-i;}});
...
function chooseTwinDigit(candidate, finalDigits, latest, akle){
  ...
  if(repeatShapeCalibration?.eligible)add(repeatShapeCalibration.digit,Math.round(6600*Math.min(1,repeatShapeCalibration.confidence/.62)),`V12 repeat-shape ${repeatShapeCalibration.shape}`);
  ...
  candidate.twinAudit = {
    ...
    decision:`V12.3 repeat-role ${repeatShapeCalibration?.shape||'-'} conf ${repeatShapeCalibration?(100*repeatShapeCalibration.confidence).toFixed(1):'0.0'}%; ${repeatAllowed?'repeat lolos':'abstain—bukti repeat belum cukup'}.`
  };
  return chosen;
}
...
function buildSlowStages(rows){
  ...
  return [
    ...
    {title:'Tahap 6 • Walk-forward router judge', icon:'⑥', delay:560, desc:'Menguji frequency, position, transform, dan carry router secara out-of-sample sebelum menyusun 6 digit, lima terkuat, AKLE, dan repeat.', items:['Hanya router ACTIVE yang boleh melakukan rescue.','TIE-BREAKER dan AUDIT-ONLY tidak boleh memaksa hasil.']}
  ];
}
...
function renderResult(r){
  ...
  const backupDigitsHtml = (r.backupFiveDigits || []).map((d,i) => `<div class="digit backup-digit">${d}<small>B${i+1}</small></div>`).join('');
  ...
  const backupPortfolioHtml = r.audit.backupPortfolio ? `<div><b>Dual-Layer Backup Portfolio</b><ul class="process-list small"><li>${escapeHtml(r.audit.backupPortfolio.title)}</li><li>Lapisan A: ${escapeHtml(r.audit.backupPortfolio.primary)}</li><li>Lapisan B: ${escapeHtml(r.audit.backupPortfolio.backup)}</li>${(r.audit.backupPortfolio.ranked||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join('')}<li>Kembar backup: ${escapeHtml(r.audit.backupPortfolio.backupTwin)}</li><li>${escapeHtml(r.audit.backupPortfolio.backupTwinNote||'')}</li><li>${escapeHtml(r.audit.backupPortfolio.note||'')}</li></ul></div>` : '';
  ...
  $('output').innerHTML = `<div class="result-block">
    <div class="final-card">
      ...
      <div class="backup-card">
        <div class="backup-head"><div><small>Lapisan B • Residual Evidence</small><b>5 Digit Backup</b></div><span class="backup-risk">Cadangan</span></div>
        <div class="digits backup-digits">${backupDigitsHtml}</div>
        <div class="backup-twin-box"><small>Kembar Backup</small><b>${r.backupTwinDigit==null?'Tidak ada kembar backup layak':`${r.backupTwinDigit}${r.backupTwinDigit}`}</b></div>
        <p>Keanggotaan Lapisan B melengkapi lima utama tanpa overlap. Urutannya dihitung dari bukti residual; level keyakinannya lebih rendah dan tidak mengubah 6D, AK, atau LE.</p>
      </div>
      <p class="tagline">Engine V12.3 memulai dari full-history scan. Lapisan A memuat konsensus utama; Lapisan B merangking kandidat residual yang kalah tipis. Kembar backup adalah runner-up berisiko tinggi, bukan pengganti kembar utama.</p>
    </div>
