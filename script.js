...
const APP_VERSION = 'IPHOEL Formula Engine V13.30 • Near-Parity Two-Step Union';
...
function twinChoiceCards(items){return `<div class="twin-choice-grid">${items.map(x=>`<div class="twin-choice"><small>Pilihan kembar ${x.choice} • ${x.sourceTwinMirror?'mirror sumber + repeat-family':x.targetTwinContinuity?'kembar target-day berulang':x.targetEchoTwin?'echo + repeat target-day':x.sourceTargetContinuity?'kontinuitas sumber–target':x.repeatPosition?'repeat-position':x.sourceContinuity?'kontinuitas sumber':x.targetDayBridge?'anchor target-day':x.repeatDepth?'kedalaman repeat seluruh replay':'digit formula'}</small><b>${x.pair}</b><span>Bentuk posisi kandidat ${x.bestShape.label} • belum mengunci posisi</span></div>`).join('')}</div>`;}
